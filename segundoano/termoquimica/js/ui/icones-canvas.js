...
SITQ.kIconText = function kIconText(ctx, desenhaIcone, texto, x, y, o = {}) {
  ctx.save();
  ...
  ctx.fillStyle = o.color || SITQ.cssVar('--text-secondary', '#7a9ab8');
  ctx.textAlign = 'left';
...
SITQ.kIconCheck = function kIconCheck(ctx, cx, cy, size, color) {
  ctx.save();
...
SITQ.kIconX = function kIconX(ctx, cx, cy, size, color) {
  ctx.save();
...
SITQ.kIconLock = function kIconLock(ctx, cx, cy, size, color) {
  ctx.save();
  ...
  ctx.lineJoin = 'round';
  const s = size / 24;
  SITQ.kRound(ctx, cx - 6.5 * s, cy - 1 * s, 13 * s, 10 * s, 2 * s);
  ctx.stroke();
  ctx.beginPath();
...
SITQ.kIconUnlock = function kIconUnlock(ctx, cx, cy, size, color) {
  ctx.save();
  ctx.strokeStyle = color || '#4ade80';
  ctx.lineWidth = Math.max(1.3, size * 0.1);
  ctx.lineCap = 'round';
...
SITQ.kIconWarning = function kIconWarning(ctx, cx, cy, size, color) {
  ctx.save();
...
SITQ.kChipIcon = function kChipIcon(ctx, desenhaIcone, texto, x, y, o = {}) {
  ctx.save();
  ...
  const w = ctx.measureText(texto).width + iconSize + gap + 16,
    h = size + 10;
  SITQ.kRound(ctx, x - w / 2, y - h / 2, w, h, h / 2);
  ...
  SITQ.kIconText(ctx, desenhaIcone, texto, x, y + .5, {
    size,
    iconSize,
    gap,
    color: o.fg || '#fff',
