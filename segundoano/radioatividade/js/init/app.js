...
SIRAD.fillOptGrid = function fillOptGrid(gridId, items, selValue) {
...
SIRAD.App = class App {
  constructor(mech) {
    ...
    this.refresh();
    SIRAD.announce(`${this.D.ACRO} carregado. Nenhum modo ativo. Escolha um modo à esquerda e ative-o para começar.`);
  ...
  _resize() {
    ...
    SIRAD.patchCtxFont(this.ctx);
    SIRAD.CANVAS_FS = SIRAD.canvasFS(this.W);
    this.lay = SIRAD.layoutMode(this.W);
  }
  ...
  _syncMobileModeUI(id) {
    ...
    if (activeTab && typeof activeTab.scrollIntoView === 'function') {
      activeTab.scrollIntoView({ inline: 'center', block: 'nearest', behavior: SIRAD.isReduced() ? 'auto' : 'smooth' });
    }
  ...
  clearMode() {
    ...
    if (typeof SIRAD.playTone === 'function') SIRAD.playTone(420, .06, .05);
    SIRAD.announce('Modo desativado. Nenhum modo ativo — escolha um modo e ative-o para voltar a simular.');
  }
  setMode(id, silent) {
    const m = this.D.MODES.find(x => x.id === id);
    if (!m) return;
    ...
    if (!silent) {
      SIRAD.playTone(760, .06, .05);
      SIRAD.announce(`Modo ${m.nome} selecionado. A animação começa em 2 segundos. ${(m.info || '').split('.')[0]}.`);
    }
  }
  ...
  _bindPanelArea(sb) {
    sb.addEventListener('click', e => {
      ...
      if (hdr) {
        ...
        SIRAD.playTone(exp ? 500 : 750, .06, .04);
        return;
      }
  ...
  fmtOut(inp, v) {
    ...
    const txt = ehLog ? SIRAD.fmtCientifico(val, 1) : SIRAD.fmt(val, casas);
    return txt + (inp.dataset.unit ? ' ' + inp.dataset.unit : '');
  }
  _param(k, v) {
    const r = this.mech.setParam(k, v) || {};
    if (r.warn) SIRAD.announce(r.warn, 'assertive');
    if (r.say) SIRAD.announce(r.say);
    this.refresh();
  }
  ...
  _bindHeader() {
    ...
    if (logo) logo.addEventListener('click', () => {
      ...
      SIRAD.playTone(660, .09, .06);
      SIRAD.announce('Você sabia? ' + fato);
    });
    document.addEventListener('keydown', e => {
      if (!e.altKey) return;
  ...
  _loop() {
    ...
    const dt = SIRAD.clamp((now - this._last) / 1000, 0, .05);
    this._last = now;
...
window.addEventListener('DOMContentLoaded', () => new SIRAD.App(new SIRAD.Mech(window.SIM_DATA)));