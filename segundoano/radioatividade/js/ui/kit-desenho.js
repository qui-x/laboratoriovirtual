...
SIRAD.kRound = function kRound(ctx, x, y, w, h, r) {
...
SIRAD.kLabel = function kLabel(ctx, text, x, y, o = {}) {
  ctx.save();
  ctx.font = `${o.bold ? '700 ' : ''}${o.size || 12}px ${o.mono ? "'Consolas','Monaco',monospace" : "'Segoe UI',system-ui,sans-serif"}`;
  ctx.fillStyle = o.color || SIRAD.cssVar('--text-secondary', '#7a9ab8');
...
SIRAD.kChip = function kChip(ctx, text, x, y, o = {}) {
  ctx.save();
  ctx.font = `${o.size || 11}px 'Segoe UI',system-ui,sans-serif`;
  const w = ctx.measureText(text).width + 14,
    h = (o.size || 11) + 10;
  SIRAD.kRound(ctx, x - w / 2, y - h / 2, w, h, h / 2);
...
SIRAD.kArrow = function kArrow(ctx, x1, y1, x2, y2, o = {}) {
  const head = o.head || 7,
    ang = Math.atan2(y2 - y1, x2 - x1);
  ctx.save();
  ctx.strokeStyle = ctx.fillStyle = o.color || SIRAD.cssVar('--text-secondary');
 ...
 */
SIRAD.kAxes = function kAxes(ctx, cfg) {
  const {
    x,
    y,
    w,
  ...
  const grid = SIRAD.cssVar('--border', '#1c2e44'),
    txt = SIRAD.cssVar('--text-muted', '#3d566e');
  ctx.save();
  ctx.lineWidth = 1;
  (cfg.xticks || []).forEach(t => {
    ...
    ctx.stroke();
    ctx.globalAlpha = 1;
    SIRAD.kLabel(ctx, cfg.fmtx ? cfg.fmtx(t) : SIRAD.fmt(t, 0), px(t), y + h + 11, {
      size: 10,
      color: txt,
      mono: true
    });
  ...
  (cfg.yticks || []).forEach(t => {
    ...
    ctx.stroke();
    ctx.globalAlpha = 1;
    SIRAD.kLabel(ctx, cfg.fmty ? cfg.fmty(t) : SIRAD.fmt(t, 0), x - 6, py(t), {
      size: 10,
      color: txt,
      align: 'right',
      mono: true
  ...
  ctx.lineTo(x + w, y + h);
  ctx.stroke();
  if (cfg.xlab) SIRAD.kLabel(ctx, cfg.xlab, x + w / 2, y + h + 26, {
    size: 11,
...
SIRAD.kLine = function kLine(ctx, pts, px, py, o = {}) {
  if (!pts.length) return;
  ctx.save();
  ctx.strokeStyle = o.color || SIRAD.cssVar('--accent-main');
 ...
 */
SIRAD.kBeaker = function kBeaker(ctx, cx, topY, w, h, level, liquidColor, o = {}) {
  const x = cx - w / 2,
    glass = SIRAD.cssVar('--glass', 'rgba(148,163,184,.38)');
  ...
  if (lh > 1) {
    ...
    SIRAD.kRound(ctx, x + 3, ly, w - 6, lh, 4);
    ctx.fill();
    // menisco
    ctx.globalAlpha = .5;
  ...
  ctx.stroke();
  if (o.rotulo) SIRAD.kLabel(ctx, o.rotulo, cx, topY + h + 14, {
    size: 11,
    color: SIRAD.cssVar('--text-secondary')
  });
  ctx.restore();
...
SIRAD.kThermo = function kThermo(ctx, x, topY, h, t, tmin, tmax, o = {}) {
  const frac = Math.max(0, Math.min(1, (t - tmin) / (tmax - tmin)));
  const bulbR = 8,
    tubeW = 7;
  const tubeTop = topY,
    tubeBot = topY + h - bulbR * 2;
  const merc = o.color || SIRAD.cssVar('--accent-exo', '#f87171');
  ctx.save();
  ctx.fillStyle = SIRAD.cssVar('--bg-void', '#080c14');
  ctx.strokeStyle = SIRAD.cssVar('--glass');
  ctx.lineWidth = 2;
  SIRAD.kRound(ctx, x - tubeW / 2, tubeTop, tubeW, h - bulbR, tubeW / 2);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
  ...
  ctx.fillStyle = merc;
  SIRAD.kRound(ctx, x - 2.2, tubeBot - mh, 4.4, mh + bulbR, 2.2);
  ctx.fill();
  for (let i = 0; i <= 4; i++) {
    ...
    ctx.stroke();
    if (o.escala !== false) SIRAD.kLabel(ctx, SIRAD.fmt(tmin + i / 4 * (tmax - tmin), 0), x + tubeW / 2 + 9, yy, {
      size: 9,
  ...
  if (o.rotulo !== false) SIRAD.kChip(ctx, `${SIRAD.fmt(t, o.casas != null ? o.casas : 0)} °C`, x, tubeTop - 14, {
    bg: 'rgba(0,0,0,.45)',
    fg: merc,
    size: 11,
    bold: true
...
SIRAD.kFlame = function kFlame(ctx, x, y, s, time) {
  const flu = SIRAD.isReduced() ? 0 : Math.sin(time * 9) * s * .06;
  ctx.save();
  const g = ctx.createRadialGradient(x, y - s * .5, s * .1, x, y - s * .5, s);
  g.addColorStop(0, SIRAD.cssVar('--flame-b', '#fde047'));
  g.addColorStop(1, SIRAD.cssVar('--flame-a', '#f97316'));
  ctx.fillStyle = g;
  ctx.beginPath();
  ctx.moveTo(x - s * .45, y);
...
SIRAD.kParticles = function kParticles(arr, n, box, speed, dt) {
  ...
  const v = SIRAD.isReduced() ? 0 : speed;
  arr.forEach(p => {
    p.x += p.vx * v * dt;
    p.y += p.vy * v * dt;
    if (p.x < box.x) {
...
SIRAD.kDrawParticles = function kDrawParticles(ctx, arr, r, color, alpha) {
  ctx.save();
  ctx.fillStyle = color;
...
SIRAD.kBubbles = function kBubbles(arr, dt, box, rate, o = {}) {
  if (!SIRAD.isReduced() && Math.random() < rate * dt) {
    arr.push({
...
SIRAD.kDrawBubbles = function kDrawBubbles(ctx, arr, color) {
  ctx.save();
...
SIRAD.kFlowDots = function kFlowDots(ctx, pts, phase, n, color, o = {}) {
  const segs = [];
  let total = 0;
  ...
  for (let k = 0; k < n; k++) {
    ...
    for (const s of segs) {
      if (d <= s.L) {
        ...
        ctx.fill();
        if (o.rotulo && k === 0) SIRAD.kLabel(ctx, 'e⁻', s.p[0] + s.dx * t, s.p[1] + s.dy * t - 9, {
          size: 9,
          color,
          mono: true
        });
...
SIRAD.kMix = function kMix(h1, h2, t) {
  const p = h => [parseInt(h.slice(1, 3), 16), parseInt(h.slice(3, 5), 16), parseInt(h.slice(5, 7), 16)];
  const a = p(h1),
    b = p(h2);
  return `rgb(${Math.round(SIRAD.lerp(a[0], b[0], t))},${Math.round(SIRAD.lerp(a[1], b[1], t))},${Math.round(SIRAD.lerp(a[2], b[2], t))})`;
};
/** Interpolação linear em tabela de pontos [[x,y],...] ordenada por x. */
SIRAD.kInterp = function kInterp(pts, x) {
