...
SIRAD.kIconText = function kIconText(ctx, desenhaIcone, texto, x, y, o = {}) {
  ctx.save();
...
SIRAD.kIconCheck = function kIconCheck(ctx, cx, cy, size, color) {
  ctx.save();
...
SIRAD.kIconX = function kIconX(ctx, cx, cy, size, color) {
  ctx.save();
...
SIRAD.kIconLock = function kIconLock(ctx, cx, cy, size, color) {
  ctx.save();
  ...
  ctx.lineJoin = 'round';
  const s = size / 24;
  SIRAD.kRound(ctx, cx - 6.5 * s, cy - 1 * s, 13 * s, 10 * s, 2 * s);
  ctx.stroke();
  ctx.beginPath();
...
SIRAD.kIconUnlock = function kIconUnlock(ctx, cx, cy, size, color) {
  ctx.save();
  ctx.strokeStyle = color || '#4ade80';
  ctx.lineWidth = Math.max(1.3, size * 0.1);
  ctx.lineCap = 'round';
...
SIRAD.kIconWarning = function kIconWarning(ctx, cx, cy, size, color) {
  ctx.save();
...
SIRAD.kChipIcon = function kChipIcon(ctx, desenhaIcone, texto, x, y, o = {}) {
  ctx.save();
  ...
  SIRAD.kIconText(ctx, desenhaIcone, texto, x, y + .5, {
    size,
    iconSize,
    gap,
    color: o.fg || '#fff',
