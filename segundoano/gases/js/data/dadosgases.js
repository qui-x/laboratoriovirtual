...
window.SIM_DATA = {
  ...
  MODES: [
    ...
    {
      id: 'mistura', sigla: 'Pᵢ = xᵢ·P', nome: 'Mistura de Gases', sub: 'Dalton, Amagat e fração molar',
      hint: 'Despeje dois ou três gases no mesmo recipiente e veja a pressão parcial de cada um ser MEDIDA pelos choques nas paredes.',
      info: 'Numa mistura de gases ideais, cada gás se comporta como se estivesse sozinho no recipiente: ocupa todo o volume e exerce sua própria pressão parcial. A Lei de Dalton diz que a pressão total é a SOMA das parciais: P = P₁ + P₂ + P₃. E cada parcial é a fração molar do gás vezes a total: Pᵢ = xᵢ·P, com xᵢ = nᵢ/n_total. A Lei de Amagat é a mesma ideia com volumes: Vᵢ = xᵢ·V. Aqui as parciais não são calculadas por fórmula — são MEDIDAS separando, espécie por espécie, o impulso que cada partícula entrega às paredes. Compare a soma das parciais medidas com a total medida: é a Lei de Dalton emergindo do modelo cinético. Repare também que, à mesma temperatura, o gás leve é visivelmente mais rápido que o pesado, mas os dois têm a MESMA energia cinética média — e por isso a pressão parcial depende só de quantos mols existem, nunca da massa molar.',
      formula: 'P = P₁ + P₂ + ⋯   ·   Pᵢ = xᵢ·P   ·   xᵢ = nᵢ/n_total',
      formulaNote: 'Dalton (pressões parciais) e Amagat (volumes parciais, Vᵢ = xᵢ·V) valem para gases ideais que não reagem entre si. A massa molar aparente da mistura é M̄ = Σxᵢ·Mᵢ, e a densidade da mistura sai de d = P·M̄/(R·T). A soma das frações molares é sempre 1.',
      icon: '<svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242" /><path d="M16 17H7" /><path d="M17 21H9" /></svg>',
      def: 'Cada gás da mistura exerce sua própria pressão parcial, e a total é a soma delas — proporcional apenas à quantidade de mols de cada um.',
      fatos: [
        { l: 'Lei de Dalton',  v: 'P = ΣPᵢ' },
        { l: 'Pressão parcial', v: 'Pᵢ = xᵢ·P' },
        { l: 'Lei de Amagat',  v: 'Vᵢ = xᵢ·V' },
        { l: 'Parciais',       v: 'medidas, não calculadas' },
      ],
      canvasInteracao: 'Ajuste os mols de cada gás e acompanhe as barras de pressão parcial medida; a soma delas fecha com a pressão total do manômetro.',
      recomendados: ['Ar (N₂ + O₂)', 'H₂ + SF₆', 'He + CO₂'],
      overlay: 'Mistura de Gases',
      panels: ['panel-mix-gases', 'panel-mix-param', 'panel-mix-acoes'], primary: 'mix-status',
    },
  ],

  /* ── Misturas prontas do modo Mistura (fração molar aproximada) ── */
  MISTURAS: [
    { rot: 'Ar atmosférico', tag: '78 % N₂ + 21 % O₂ + 1 % Ar — a mistura em que você está agora',
      itens: [['n2', 2.34], ['o2', 0.63], ['ar', 0.03]] },
    { rot: 'H₂ + SF₆', tag: 'Massas molares em extremos opostos: mesmas parciais com velocidades muito diferentes',
      itens: [['h2', 1.5], ['sf6', 1.5]] },
    { rot: 'He + CO₂', tag: 'O leve e o denso: por que o CO₂ se acumula no fundo de poços',
      itens: [['he', 1.5], ['co2', 1.5]] },
    { rot: 'Gás de síntese', tag: 'CO + H₂ na proporção 1:2 — matéria-prima do metanol industrial',
      itens: [['co', 1], ['h2', 2]] },
  ],
  ...
  CURIOSIDADES: [
    ...
    'A pressão parcial de O₂ no ar é 0,21 atm ao nível do mar. No topo do Everest a pressão total cai a ~0,33 atm, então a parcial de O₂ vira ~0,07 atm — a fração molar continua 21 %, mas o que falta é pressão.',
    'Cilindros de mergulho profundo usam trimix (He + O₂ + N₂): a ideia é baixar a pressão PARCIAL de N₂ e de O₂, que a altas profundidades ficariam tóxicas.',
    'A massa molar aparente do ar é ≈ 29 g/mol — a média ponderada pelas frações molares de N₂ (28) e O₂ (32). É esse 29 que define se um gás sobe ou desce no ar.',
  ],
};
