...
SIGAS.fillOptGrid = function fillOptGrid(gridId, items, selValue) {
...
SIGAS.App = class App {
  constructor(mech) {
    ...
    this.refresh();
    SIGAS.announce(`${this.D.ACRO} carregado. Nenhum modo ativo. Escolha um modo à esquerda e ative-o para começar.`);
  ...
  _resize() {
    ...
    SIGAS.patchCtxFont(this.ctx);
    SIGAS.CANVAS_FS = SIGAS.canvasFS(this.W);
    this.lay = SIGAS.layoutMode(this.W);
  }
  ...
  _syncMobileModeUI(id) {
    ...
    if (activeTab && typeof activeTab.scrollIntoView === 'function') {
      activeTab.scrollIntoView({ inline: 'center', block: 'nearest', behavior: SIGAS.isReduced() ? 'auto' : 'smooth' });
    }
  ...
  clearMode() {
    ...
    if (typeof SIGAS.playTone === 'function') SIGAS.playTone(420, .06, .05);
    SIGAS.announce('Modo desativado. Nenhum modo ativo — escolha um modo e ative-o para voltar a simular.');
  }
  setMode(id, silent) {
    const m = this.D.MODES.find(x => x.id === id);
    if (!m) return;
    ...
    if (!silent) {
      SIGAS.playTone(760, .06, .05);
      SIGAS.announce(`Modo ${m.nome} selecionado. A animação começa em 2 segundos. ${(m.info || '').split('.')[0]}.`);
    }
  }
  ...
  _bindPanelArea(sb) {
    sb.addEventListener('click', e => {
      ...
      if (hdr) {
        ...
        SIGAS.playTone(exp ? 500 : 750, .06, .04);
        return;
      }
  ...
  fmtOut(inp, v) {
    ...
    const txt = ehLog ? SIGAS.fmtCientifico(val, 1) : SIGAS.fmt(val, casas);
    return txt + (inp.dataset.unit ? ' ' + inp.dataset.unit : '');
  }
  _param(k, v) {
    const r = this.mech.setParam(k, v) || {};
    if (r.warn) SIGAS.announce(r.warn, 'assertive');
    if (r.say) SIGAS.announce(r.say);
    this.refresh();
  }
  ...
  _bindHeader() {
    ...
    if (logo) logo.addEventListener('click', () => {
      ...
      SIGAS.playTone(660, .09, .06);
      SIGAS.announce('Você sabia? ' + fato);
    });
    document.addEventListener('keydown', e => {
      if (!e.altKey) return;
  ...
  _loop() {
    ...
    const dt = SIGAS.clamp((now - this._last) / 1000, 0, .05);
    this._last = now;
...
window.addEventListener('DOMContentLoaded', () => new SIGAS.App(new SIGAS.Mech(window.SIM_DATA)));