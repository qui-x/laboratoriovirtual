...
SICIN.fillOptGrid = function fillOptGrid(gridId, items, selValue) {
...
SICIN.App = class App {
  constructor(mech) {
    ...
    this.refresh();
    SICIN.announce(`${this.D.ACRO} carregado. Nenhum modo ativo. Escolha um modo à esquerda e ative-o para começar.`);
  ...
  _resize() {
    ...
    SICIN.patchCtxFont(this.ctx);
    SICIN.CANVAS_FS = SICIN.canvasFS(this.W);
    this.lay = SICIN.layoutMode(this.W);
  }
  ...
  _bindPanelArea(sb) {
    sb.addEventListener('click', e => {
      ...
      if (e.target.closest('#hdr-ord .panel-icon')) {
        ...
        this._ordIconT = now;
        this._ordIconN = (this._ordIconN || 0) + 1;
        if (this._ordIconN >= 5) {
          this._ordIconN = 0;
          ...
          if (this.mech.ordSecreto) {
            ...
            this.mech.ordTrel = 0;
            this.mech.ordRunning = false;
            SICIN.announce('Desafio da ordem escondida ativado! A ordem verdadeira foi sorteada e escondida — descubra testando as três linearizações.');
          } else {
            SICIN.announce('Desafio da ordem escondida desativado.');
          }
          SICIN.playTone(this.mech.ordSecreto ? 900 : 500, .1, .06);
          this.refresh();
        }
      }
      ...
      if (hdr) {
        ...
        SICIN.playTone(exp ? 500 : 750, .06, .04);
        return;
      }
  ...
  fmtOut(inp, v) {
    ...
    const txt = ehLog ? SICIN.fmtCientifico(val, 1) : SICIN.fmt(val, casas);
    return txt + (inp.dataset.unit ? ' ' + inp.dataset.unit : '');
  }
  _param(k, v) {
    const r = this.mech.setParam(k, v) || {};
    if (r.warn) SICIN.announce(r.warn, 'assertive');
    if (r.say) SICIN.announce(r.say);
    this.refresh();
  }
  ...
  refresh() {
    ...
    if (this.mode && this.mode.id === 'ordem') {
      ...
      if (btn) btn.innerHTML = this.mech.ordRunning ? '<span aria-hidden="true"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="14" y="3" width="5" height="18" rx="1" /><rect x="5" y="3" width="5" height="18" rx="1" /></svg></span> Pausar corrida' : '<span aria-hidden="true"><svg viewBox="0 0 24 24" width="1em" height="1em" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 5a2 2 0 0 1 3.008-1.728l11.997 6.998a2 2 0 0 1 .003 3.458l-12 7A2 2 0 0 1 5 19z" /></svg></span> Iniciar corrida';
    }
  }
  ...
  _refreshResultChart() {
    ...
    if (temGrafico) {
      const dpr = window.devicePixelRatio || 1;
      const w = rc.clientWidth || 260,
        h = rc.clientHeight || 160;
      if (rc.width !== w * dpr || rc.height !== h * dpr) {
        rc.width = w * dpr;
        rc.height = h * dpr;
      }
  ...
  _bindHeader() {
    ...
    if (logo) logo.addEventListener('click', () => {
      ...
      SICIN.playTone(660, .09, .06);
      SICIN.announce('Você sabia? ' + fato);
    });
    document.addEventListener('keydown', e => {
      if (!e.altKey) return;
  ...
  _loop() {
    const now = performance.now();
    const dt = SICIN.clamp((now - this._last) / 1000, 0, .05);
    this._last = now;
...
window.addEventListener('DOMContentLoaded', () => new SICIN.App(new SICIN.Mech(window.SIM_DATA)));