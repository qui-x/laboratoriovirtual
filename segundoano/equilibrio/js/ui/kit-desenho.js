...
SIEQ.kRound = function kRound(ctx, x, y, w, h, r) {
...
SIEQ.kLabel = function kLabel(ctx, text, x, y, o = {}) {
  ctx.save();
  ctx.font = `${o.bold ? '700 ' : ''}${o.size || 12}px ${o.mono ? "'Consolas','Monaco',monospace" : "'Segoe UI',system-ui,sans-serif"}`;
  ctx.fillStyle = o.color || SIEQ.cssVar('--text-secondary', '#7a9ab8');
...
SIEQ.kChip = function kChip(ctx, text, x, y, o = {}) {
  ctx.save();
  ctx.font = `${o.size || 11}px 'Segoe UI',system-ui,sans-serif`;
  const w = ctx.measureText(text).width + 14,
    h = (o.size || 11) + 10;
  SIEQ.kRound(ctx, x - w / 2, y - h / 2, w, h, h / 2);
...
SIEQ.kArrow = function kArrow(ctx, x1, y1, x2, y2, o = {}) {
  const head = o.head || 7,
    ang = Math.atan2(y2 - y1, x2 - x1);
  ctx.save();
  ctx.strokeStyle = ctx.fillStyle = o.color || SIEQ.cssVar('--text-secondary');
 ...
 */
SIEQ.kAxes = function kAxes(ctx, cfg) {
  const {
    x,
    y,
    w,
  ...
  const grid = SIEQ.cssVar('--border', '#1c2e44'),
    txt = SIEQ.cssVar('--text-muted', '#3d566e');
  ctx.save();
  ctx.lineWidth = 1;
  (cfg.xticks || []).forEach(t => {
    ...
    ctx.stroke();
    ctx.globalAlpha = 1;
    SIEQ.kLabel(ctx, cfg.fmtx ? cfg.fmtx(t) : SIEQ.fmt(t, 0), px(t), y + h + 11, {
      size: 10,
      color: txt,
      mono: true
    });
  ...
  (cfg.yticks || []).forEach(t => {
    ...
    ctx.stroke();
    ctx.globalAlpha = 1;
    SIEQ.kLabel(ctx, cfg.fmty ? cfg.fmty(t) : SIEQ.fmt(t, 0), x - 6, py(t), {
      size: 10,
      color: txt,
      align: 'right',
      mono: true
  ...
  ctx.lineTo(x + w, y + h);
  ctx.stroke();
  if (cfg.xlab) SIEQ.kLabel(ctx, cfg.xlab, x + w / 2, y + h + 26, {
    size: 11,
...
SIEQ.kLine = function kLine(ctx, pts, px, py, o = {}) {
  if (!pts.length) return;
  ctx.save();
  ctx.strokeStyle = o.color || SIEQ.cssVar('--accent-main');
 ...
 */
SIEQ.kBeaker = function kBeaker(ctx, cx, topY, w, h, level, liquidColor, o = {}) {
  const x = cx - w / 2,
    glass = SIEQ.cssVar('--glass', 'rgba(148,163,184,.38)');
  ...
  if (lh > 1) {
    ...
    SIEQ.kRound(ctx, x + 3, ly, w - 6, lh, 4);
    ctx.fill();
    // menisco
    ctx.globalAlpha = .5;
  ...
  ctx.stroke();
  if (o.rotulo) SIEQ.kLabel(ctx, o.rotulo, cx, topY + h + 14, {
    size: 11,
    color: SIEQ.cssVar('--text-secondary')
  });
  ctx.restore();
...
SIEQ.kThermo = function kThermo(ctx, x, topY, h, t, tmin, tmax, o = {}) {
  const frac = Math.max(0, Math.min(1, (t - tmin) / (tmax - tmin)));
  const bulbR = 8,
    tubeW = 7;
  const tubeTop = topY,
    tubeBot = topY + h - bulbR * 2;
  const merc = o.color || SIEQ.cssVar('--accent-exo', '#f87171');
  ctx.save();
  ctx.fillStyle = SIEQ.cssVar('--bg-void', '#080c14');
  ctx.strokeStyle = SIEQ.cssVar('--glass');
  ctx.lineWidth = 2;
  SIEQ.kRound(ctx, x - tubeW / 2, tubeTop, tubeW, h - bulbR, tubeW / 2);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
  ...
  ctx.fillStyle = merc;
  SIEQ.kRound(ctx, x - 2.2, tubeBot - mh, 4.4, mh + bulbR, 2.2);
  ctx.fill();
  for (let i = 0; i <= 4; i++) {
    ...
    ctx.stroke();
    if (o.escala !== false) SIEQ.kLabel(ctx, SIEQ.fmt(tmin + i / 4 * (tmax - tmin), 0), x + tubeW / 2 + 9, yy, {
      size: 9,
  ...
  if (o.rotulo !== false) SIEQ.kChip(ctx, `${SIEQ.fmt(t, o.casas != null ? o.casas : 0)} °C`, x, tubeTop - 14, {
    bg: 'rgba(0,0,0,.45)',
    fg: merc,
    size: 11,
    bold: true
...
SIEQ.kFlame = function kFlame(ctx, x, y, s, time) {
  const flu = SIEQ.isReduced() ? 0 : Math.sin(time * 9) * s * .06;
  ctx.save();
  const g = ctx.createRadialGradient(x, y - s * .5, s * .1, x, y - s * .5, s);
  g.addColorStop(0, SIEQ.cssVar('--flame-b', '#fde047'));
  g.addColorStop(1, SIEQ.cssVar('--flame-a', '#f97316'));
  ctx.fillStyle = g;
  ctx.beginPath();
  ctx.moveTo(x - s * .45, y);
...
SIEQ.kParticles = function kParticles(arr, n, box, speed, dt) {
  ...
  const v = SIEQ.isReduced() ? 0 : speed;
  arr.forEach(p => {
    p.x += p.vx * v * dt;
    p.y += p.vy * v * dt;
    if (p.x < box.x) {
...
SIEQ.kDrawParticles = function kDrawParticles(ctx, arr, r, color, alpha) {
  ctx.save();
  ctx.fillStyle = color;
...
SIEQ.kBubbles = function kBubbles(arr, dt, box, rate, o = {}) {
  if (!SIEQ.isReduced() && Math.random() < rate * dt) {
    arr.push({
...
SIEQ.kDrawBubbles = function kDrawBubbles(ctx, arr, color) {
  ctx.save();
...
SIEQ.kFlowDots = function kFlowDots(ctx, pts, phase, n, color, o = {}) {
  const segs = [];
  let total = 0;
  ...
  for (let k = 0; k < n; k++) {
    ...
    for (const s of segs) {
      if (d <= s.L) {
        ...
        ctx.fill();
        if (o.rotulo && k === 0) SIEQ.kLabel(ctx, 'e⁻', s.p[0] + s.dx * t, s.p[1] + s.dy * t - 9, {
          size: 9,
          color,
          mono: true
        });
...
SIEQ.kMix = function kMix(h1, h2, t) {
  const p = h => [parseInt(h.slice(1, 3), 16), parseInt(h.slice(3, 5), 16), parseInt(h.slice(5, 7), 16)];
  const a = p(h1),
    b = p(h2);
  return `rgb(${Math.round(SIEQ.lerp(a[0], b[0], t))},${Math.round(SIEQ.lerp(a[1], b[1], t))},${Math.round(SIEQ.lerp(a[2], b[2], t))})`;
};
/** Interpolação linear em tabela de pontos [[x,y],...] ordenada por x. */
SIEQ.kInterp = function kInterp(pts, x) {
