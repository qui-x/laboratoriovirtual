...
SIEQ.kIconText = function kIconText(ctx, desenhaIcone, texto, x, y, o = {}) {
  ctx.save();
...
SIEQ.kIconCheck = function kIconCheck(ctx, cx, cy, size, color) {
  ctx.save();
...
SIEQ.kIconX = function kIconX(ctx, cx, cy, size, color) {
  ctx.save();
...
SIEQ.kIconLock = function kIconLock(ctx, cx, cy, size, color) {
  ctx.save();
  ...
  ctx.lineJoin = 'round';
  const s = size / 24;
  SIEQ.kRound(ctx, cx - 6.5 * s, cy - 1 * s, 13 * s, 10 * s, 2 * s);
  ctx.stroke();
  ctx.beginPath();
...
SIEQ.kIconUnlock = function kIconUnlock(ctx, cx, cy, size, color) {
  ctx.save();
  ctx.strokeStyle = color || '#4ade80';
  ctx.lineWidth = Math.max(1.3, size * 0.1);
  ctx.lineCap = 'round';
...
SIEQ.kIconWarning = function kIconWarning(ctx, cx, cy, size, color) {
  ctx.save();
...
SIEQ.kChipIcon = function kChipIcon(ctx, desenhaIcone, texto, x, y, o = {}) {
  ctx.save();
  ...
  SIEQ.kIconText(ctx, desenhaIcone, texto, x, y + .5, {
    size,
    iconSize,
    gap,
    color: o.fg || '#fff',
