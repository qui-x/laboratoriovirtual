...
class MechA {
  ...
  _prepCalc() {
    ...
    return {
      limite, dissMax, diss, corpo, Cnom: P.massa / VL, C, Cexp: C * err, M, Mexp: M * err,
      satFrac: limite > 0 ? diss / limite : 0,
      mSolvente, mSolucao, titulo, pct: titulo * 100, ppm, dSolucao, molalidade,
      nMol: diss / P.sol.M,
    };
  }
  _dilCalc() {
  ...
  getResults() {
    ...
    if (this.mode === 'preparo') {
      ...
      return [
        { l: 'Soluto', v: P.sol.nome.split(' (')[0] },
        { l: 'Massa molar', v: `${fmt(P.sol.M, 2)} g/mol` },
        { l: 'Soluto dissolvido', v: `${fmt(c.diss, 2)} g  =  ${fmt(c.nMol, 4)} mol` },
        { l: 'Massa de solvente', v: `${fmt(c.mSolvente, 0)} g de água` },
        { l: 'Massa de solução', v: `${fmt(c.mSolucao, 1)} g` },
        ...
        { l: 'Molaridade M = n/V', v: `${fmt(c.Mexp, 3)} mol/L` },
        { l: 'Molalidade W = n/kg solv.', v: `${fmt(c.molalidade, 4)} mol/kg` },
        { l: 'Título τ = m_soluto/m_sol.', v: fmt(c.titulo, 5) },
        { l: 'Porcentagem em massa', v: `${fmt(c.pct, 3)} % (m/m)`, cls: 'val-ok' },
        { l: 'ppm', v: `${fmt(c.ppm, 0)} ppm` },
        { l: 'Densidade da solução', v: `${fmt(c.dSolucao, 4)} g/mL` },
        { l: 'Relação C = 1000·d·τ', v: `${fmt(1000 * c.dSolucao * c.titulo, 1)} g/L (confere com C)` },
        { l: '— Saturação —', v: '' },
