...
class ThermoSim {
  constructor(canvas, onEvent) {
    ...
    this.gibbs = { r: REACOES_GIBBS[0], T: 298.15 };
  ...
  draw() {
    ...
    switch (this.mode) {
      ...
      case 'gibbs':   this.drawGibbs();   break;
    }
  }
  ...
  gibbsCalc(Topt) {
    ...
    let caso, regra;
    if (r.dH < 0 && r.dS > 0) { caso = 'ΔH < 0 e ΔS > 0'; regra = 'espontânea a QUALQUER temperatura'; }
    else if (r.dH > 0 && r.dS < 0) { caso = 'ΔH > 0 e ΔS < 0'; regra = 'NUNCA espontânea (o inverso é que é)'; }
    else if (r.dH > 0 && r.dS > 0) { caso = 'ΔH > 0 e ΔS > 0'; regra = 'espontânea só em ALTA temperatura'; }
    else { caso = 'ΔH < 0 e ΔS < 0'; regra = 'espontânea só em BAIXA temperatura'; }
    return {
      T, tds, dG, Tinv, temInversao, caso, regra,
      espontanea: dG < 0,
  ...
  drawGibbs() {
    const { ctx, W, H } = this;
    const g = this.gibbs, r = g.r, c = this.gibbsCalc();
    const est = W < 620;
    ...
    ctx.fillText(`ΔH = ${fmt(r.dH, 1)} kJ/mol   ·   ΔS = ${fmt(r.dS, 1)} J/(mol·K)`, W / 2, est ? 36 : 50, W - 20);
    ...
    ctx.save();
    ctx.fillStyle = cssVar('--accent-ok', '#4ade80'); ctx.globalAlpha = .10;
    ctx.fillRect(gx, py(0), gw, gy + gh - py(0));
    ctx.restore();
    ctx.save();
    ctx.strokeStyle = cssVar('--text-secondary'); ctx.lineWidth = 1.6;
    ctx.beginPath(); ctx.moveTo(gx, py(0)); ctx.lineTo(gx + gw, py(0)); ctx.stroke();
    ctx.restore();
    ctx.fillStyle = cssVar('--accent-ok'); ctx.textAlign = 'left';
    ctx.font = `${est ? 8 : 10}px Consolas, monospace`;
    ctx.fillText('ΔG < 0 · espontânea', gx + 6, py(0) + (est ? 12 : 15));
    ctx.fillStyle = cssVar('--accent-exo');
    ctx.fillText('ΔG > 0 · não espontânea', gx + 6, py(0) - (est ? 10 : 13));

    // ticks de T
    ctx.fillStyle = cssVar('--text-muted'); ctx.textAlign = 'center';
    ctx.font = `${est ? 8 : 9}px Consolas, monospace`;
    [0, 300, 600, 900, 1200, 1500].forEach(t => {
      ctx.fillText(String(t), px(t), gy + gh + (est ? 10 : 13));
      ctx.strokeStyle = cssVar('--border');
      ctx.beginPath(); ctx.moveTo(px(t), gy + gh); ctx.lineTo(px(t), gy + gh + 4); ctx.stroke();
    });
    ctx.fillText('Temperatura (K)', gx + gw / 2, gy + gh + (est ? 24 : 30));
    ctx.save();
    ctx.translate(gx - (est ? 32 : 52), gy + gh / 2); ctx.rotate(-Math.PI / 2);
    ctx.fillText('ΔG (kJ/mol)', 0, 0);
    ctx.restore();
    // ticks de ΔG
    ctx.textAlign = 'right';
    [-amp, -amp / 2, 0, amp / 2, amp].forEach(v => {
      ctx.fillText(fmt(v, 0), gx - 5, py(v));
    });

    // a reta
    ctx.save();
    ctx.strokeStyle = r.cor; ctx.lineWidth = 3; ctx.lineJoin = 'round';
    ctx.beginPath(); ctx.moveTo(px(Tmin), py(g0)); ctx.lineTo(px(Tmax), py(g1)); ctx.stroke();
    ctx.restore();

    // temperatura de inversão
    if (c.temInversao && c.Tinv > Tmin && c.Tinv < Tmax) {
      ctx.save();
      ctx.setLineDash([4, 4]); ctx.strokeStyle = cssVar('--accent-amber', '#fbbf24'); ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(px(c.Tinv), gy); ctx.lineTo(px(c.Tinv), gy + gh); ctx.stroke();
      ctx.restore();
      ctx.fillStyle = cssVar('--accent-amber');
      ctx.font = `700 ${est ? 9 : 11}px Consolas, monospace`;
      ctx.textAlign = 'center';
      ctx.fillText(`T inversão ${fmt(c.Tinv, 0)} K`, clamp(px(c.Tinv), gx + 50, gx + gw - 50), gy - 6);
    }

    // ponto atual
    ctx.save();
    ctx.fillStyle = c.espontanea ? cssVar('--accent-ok', '#4ade80') : cssVar('--accent-exo', '#f87171');
    ctx.beginPath(); ctx.arc(px(c.T), py(c.dG), est ? 5 : 6.5, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = '#fff'; ctx.lineWidth = 1.6; ctx.stroke();
    ctx.restore();
    ctx.fillStyle = c.espontanea ? cssVar('--accent-ok') : cssVar('--accent-exo');
    ctx.font = `700 ${est ? 10 : 12}px Consolas, monospace`;
    ctx.textAlign = 'center';
    ctx.fillText(`${fmt(c.T, 0)} K · ΔG = ${fmt(c.dG, 1)} kJ`,
      clamp(px(c.T), gx + 70, gx + gw - 70), py(c.dG) - (est ? 14 : 18));
    ...
    if (by + 60 < H) {
      ...
      const barra = (rot, val, cor, y) => {
        ...
        ctx.save();
        ctx.fillStyle = cor; ctx.globalAlpha = .8;
        if (val >= 0) ctx.fillRect(meio, y, w, est ? 12 : 15);
        else ctx.fillRect(meio - w, y, w, est ? 12 : 15);
        ctx.restore();
        ctx.fillStyle = cssVar('--text-secondary'); ctx.textAlign = 'right';
        ...
        ctx.fillText(`${fmt(val, 1)} kJ`, bx0 + bw + 6, y + (est ? 6 : 8));
      };
      // linha do zero das barras
      ctx.save();
      ctx.strokeStyle = cssVar('--border'); ctx.lineWidth = 1;
      ctx.beginPath(); ctx.moveTo(bx0 + bw / 2, by - 4); ctx.lineTo(bx0 + bw / 2, by + (est ? 46 : 56)); ctx.stroke();
      ctx.restore();
      barra('ΔH', r.dH, cssVar('--accent-exo', '#f87171'), by);
      barra('−T·ΔS', -c.tds, cssVar('--accent-cyan', '#22d3ee'), by + (est ? 16 : 20));
      barra('= ΔG', c.dG, c.espontanea ? cssVar('--accent-ok', '#4ade80') : cssVar('--accent-amber', '#fbbf24'), by + (est ? 32 : 40));
      ctx.textAlign = 'center';
...
// ══════════════════════════════════════════════════════════════════
const MODOS_ORDEM = ['calor', 'curva', 'perfil', 'hess', 'ligacao', 'gibbs'];
...
class ThermoApp {
  ...
  _buildGibbsList() {
    ...
    REACOES_GIBBS.forEach((r, i) => {
      ...
      b.setAttribute('aria-selected', i === 0 ? 'true' : 'false');
      b.innerHTML =
        `<span class="model-year" aria-hidden="true">ΔH ${r.dH > 0 ? '+' : '−'}<br>ΔS ${r.dS > 0 ? '+' : '−'}</span>` +
        `<span class="model-name">${r.nome}</span>` +
        `<span class="model-sub">${r.eq}</span>`;
      b.setAttribute('aria-label',
        `${r.nome}. Delta H de ${fmt(r.dH, 1)} quilojoules por mol, delta S de ${fmt(r.dS, 1)} joules por mol kelvin.`);
      b.addEventListener('click', () => {
        list.querySelectorAll('.model-btn').forEach(x => { x.classList.remove('active'); x.setAttribute('aria-selected', 'false'); });
        b.classList.add('active'); b.setAttribute('aria-selected', 'true');
        this.sim.gibbs.r = r;
        playTone(r.dH < 0 ? 620 : 980, .07, .06);
        this._syncOverlay();
        const desc = document.getElementById('gibbs-desc');
        if (desc) desc.textContent = r.desc;
        this.refreshResults();
        const c = this.sim.gibbsCalc();
        announce(`${r.nome} selecionada. ${c.caso}: ${c.regra}. A 25 graus, delta G vale ${fmt(c.dG, 1)} quilojoules, ou seja, ${c.espontanea ? 'espontânea' : 'não espontânea'}. ${r.desc}`);
      });
      list.appendChild(b);
    });
  ...
  _bindGibbs() {
    ...
    if (sl) {
      sl.addEventListener('input', () => {
        ...
        if (out) out.textContent = `${fmt(+sl.value, 0)} K (${fmt(+sl.value - 273.15, 0)} °C)`;
        this._syncOverlay();
        this.refreshResults();
      });
    }
    ...
    if (btn25) btn25.addEventListener('click', () => {
      ...
      announce('Temperatura de volta a 298 kelvin, ou 25 graus Celsius — a condição padrão de tabela.');
    });
  }

  _gibbsSetT(T) {
    this.sim.gibbs.T = clamp(T, 1, 1500);
    const sl = document.getElementById('gibbs-t');
    if (sl) sl.value = this.sim.gibbs.T;
    const out = document.getElementById('out-gibbs-t');
    if (out) out.textContent = `${fmt(this.sim.gibbs.T, 0)} K (${fmt(this.sim.gibbs.T - 273.15, 0)} °C)`;
    this._syncOverlay();
    this.refreshResults();
  }
  ...
  _gibbsIrInversao() {
    ...
    if (!c.temInversao) {
      playTone(300, .12, .06);
      return announce(`Esta reação não tem temperatura de inversão. ΔH e ΔS têm sinais OPOSTOS, então a reta de ΔG nunca cruza o zero: o comportamento é o mesmo em qualquer temperatura — ${c.regra}.`, 'assertive');
    }
    if (c.Tinv > 1500 || c.Tinv < 1) {
      playTone(420, .1, .06);
      return announce(`A temperatura de inversão desta reação é ${fmt(c.Tinv, 0)} kelvin, fora da faixa do controle (1 a 1500 kelvin). Na prática, isso significa que dentro de qualquer temperatura razoável ela é sempre ${c.espontanea ? 'espontânea' : 'não espontânea'}.`, 'assertive');
    }
    this._gibbsSetT(c.Tinv);
    playTone(760, .09, .06);
    const d = this.sim.gibbsCalc();
    announce(`Temperatura levada a ${fmt(c.Tinv, 0)} kelvin, ou ${fmt(c.Tinv - 273.15, 0)} graus Celsius: aqui ΔG vale ${fmt(d.dG, 2)} quilojoules, praticamente zero. É o ponto de equilíbrio, em que o termo entálpico e o entrópico se cancelam. Abaixo dele o processo é espontâneo num sentido; acima, no sentido inverso.`, 'assertive');
  }
  ...
  setMode(mode, inicial) {
    ...
    document.querySelectorAll('[data-mode]').forEach(b => {
      ...
      b.title = on ? ('Desativar ' + (MODO_NOME[b.dataset.mode] || ''))
                   : ('Ativar ' + (MODO_NOME[b.dataset.mode] || '') + ' no canvas');
  ...
  static get ICONES() {
    return { calor:'🌡️', curva:'📈', perfil:'⛰️', hess:'🧩', ligacao:'⚛️', gibbs:'🧭' };
  }
  ...
  _syncOverlay() {
    ...
    if (ico) ico.textContent = (ThermoApp.ICONES[this.mode] || '');
  ...
  refreshResults() {
    ...
    if (this.mode === 'gibbs') {
      const r = this.sim.gibbs.r, c = this.sim.gibbsCalc();
      row('Reação', r.nome);
      row('ΔH', `${fmt(r.dH, 1)} kJ/mol`, r.dH < 0 ? 'val-exo' : 'val-endo');
      row('ΔS', `${fmt(r.dS, 1)} J/(mol·K)`, r.dS > 0 ? 'val-ok' : 'val-endo');
      row('Temperatura', `${fmt(c.T, 1)} K  =  ${fmt(c.T - 273.15, 1)} °C`);
      ...
      row('T·ΔS (convertido)', `${fmt(c.tds, 2)} kJ/mol`);
      row('ΔG = ΔH − T·ΔS', `${fmt(c.dG, 2)} kJ/mol`, c.espontanea ? 'val-exo' : 'val-endo');
      row('Veredito', c.espontanea ? 'ESPONTÂNEA (ΔG < 0)' : (Math.abs(c.dG) < 0.5 ? 'EQUILÍBRIO (ΔG ≈ 0)' : 'NÃO espontânea (ΔG > 0)'),
        c.espontanea ? 'val-ok' : 'val-endo');
      row('Par de sinais', c.caso);
      row('Comportamento', c.regra, 'val-ok');
      row('T de inversão', c.temInversao ? `${fmt(c.Tinv, 1)} K  =  ${fmt(c.Tinv - 273.15, 1)} °C` : 'não existe (sinais opostos)',
        c.temInversao ? 'val-ok' : '');
