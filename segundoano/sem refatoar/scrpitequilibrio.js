...
class MechA {
  ...
  action(name) {
    ...
    if (name === 'lch-reset') {
      ...
      this.nAr = 0; this.cat = false; this.relax = 0;
      this._equilibrar(true);   // salto: nao houve perturbacao para observar
      return announce('Frasco reiniciado a 25 graus e volume normal, sem gás inerte e sem catalisador.');
    }
    const P = {
      ...
      'inerte': () => { this.nAr = (this.nAr || 0) + 0.05; this.perturb = 'adição de gás inerte (argônio) a volume constante'; },
      'catal':  () => { this.cat = !this.cat; this.perturb = this.cat ? 'entrada do catalisador' : 'retirada do catalisador'; },
    };
    if (P[name]) {
      ...
      let sentido = dir > 0 ? 'para a direita, formando mais NO₂ castanho'
      ...
      if (name === 'inerte') {
        ...
        extra = this.cat
          ? ' O catalisador acelera IGUALMENTE as reações direta e inversa: o equilíbrio é alcançado mais rápido, no mesmo ponto. Ele não altera Kc nem o rendimento.'
          : ' Catalisador retirado. O ponto de equilíbrio é o mesmo — só a pressa muda.';
      }
      announce(`Perturbação: ${this.perturb}. O equilíbrio se desloca ${sentido}. Concentração de NO₂ vai de ${fmt(antes, 4)} para ${fmt(depois, 4)} mol por litro.${extra}`);
    }
  }
  ...
  _lchStep(dt) {
    ...
    const K = this._K(this.T);
    const kf = 1.6, kr = kf / Math.max(1e-12, K);
    const passos = 10, h = Math.min(dt, .05) / passos;
    for (let i = 0; i < passos; i++) {
      const a = this.nN2O4 / this.V, b = this.nNO2 / this.V;
  ...
  update(dt, app) {
    this.fase += dt;
    if (this.modo === 'lechatelier') { this._lchStep(dt); return; }
  ...
  _drawAtg(ctx, W, H) {
    ...
    const A = kAxes(ctx, {
      x: mx, y: 40, w: gw, h: gh, xmin: 0, xmax: tmax, ymin: 0, ymax: cmax,
  ...
  _drawLch(ctx, W, H) {
    ...
    const est = isEstreito(W);
    const esc = clamp(W / 900, .78, 1.6);   // escala geral do desenho
    ...
    const fw = clamp(70 * Math.sqrt(this.V) * esc, 44, 260);
    ...
    if (gw > 150 && gh > 90) {
      ...
      const A = kAxes(ctx, {
        x: gx, y: gy, w: gw, h: gh, xmin: 0, xmax: Math.max(20, this.strip.length), ymin: 0, ymax: cmax,
      ...
      kChip(ctx, '[N₂O₄]', gx + 42, gy - 12, { fg: cssVar('--text-secondary'), size: 10 });
      kChip(ctx, '[NO₂]', gx + 110, gy - 12, { fg: '#c2410c', size: 10 });
      kChip(ctx, `K(T) = ${K.toExponential(2)}`, gx + gw / 2, gy + gh + 34,
  ...
  _drawQk(ctx, W, H) {
    ...
    // tela estreita.
    const bw = Math.max(200, W - (isEstreito(W) ? 48 : 120));
  ...
  getResults() {
    ...
    if (this.modo === 'lechatelier') {
      return [
        ...
        { l: 'Q = [NO₂]²/[N₂O₄]', v: (this._concNO2() ** 2 / Math.max(1e-12, this._concN2O4())).toExponential(3) },
        { l: 'Kc na T atual', v: this._K(this.T).toExponential(3) },
        { l: 'ΔH', v: '+' + this.D.NO2.dh + ' kJ/mol', cls: 'val-endo' },
        { l: 'Gás inerte (Ar)', v: this.nAr > 0 ? `${fmt(this.nAr, 3)} mol — não desloca` : 'nenhum' },
        { l: 'Catalisador', v: this.cat ? 'presente — não desloca, só acelera' : 'ausente' },
        { l: 'Estado', v: this.relax ? 'deslocando…' : 'em equilíbrio', cls: this.relax ? 'val-endo' : 'val-ok' },
...
class MechB {
  ...
  _drawEsc(ctx, W, H) {
    ...
    const x0 = (W - bw) / 2, y = est ? 58 : 70;
    const bh = Math.round(clamp(34 * (W / 900), 30, 56));
  ...
  _drawCalc(ctx, W, H) {
    ...
    ctx.restore();
    kChip(ctx, `α = ${fmt(r.alpha * 100, 2)} % ionizado`, bx, bcy + 32,
    ...
    linhas.forEach((l, i) => {
      ...
      kLabel(ctx, l[1], px + pw, y, { size: 12, align: 'right', bold: true, mono: true, color: cssVar('--text-primary') });
      ctx.save();
      ctx.strokeStyle = cssVar('--border', '#1c2e44');
      ctx.beginPath(); ctx.moveTo(px, y + dy * .43); ctx.lineTo(px + pw, y + dy * .43); ctx.stroke();
      ctx.restore();
    });
  ...
  _drawTit(ctx, W, H) {
    ...
    if (gw > 160 && gh > 90) {
      const A = kAxes(ctx, {
        x: gx, y: gy, w: gw, h: gh, xmin: 0, xmax: 50, ymin: 0, ymax: 14,
...
// ══════════════════════════════════════════════════════════════════
// MechC — os três equilíbrios que faltavam no SIEQ
// ══════════════════════════════════════════════════════════════════
// Kps (produto de solubilidade) · solução tampão · hidrólise salina.
//
// Classe nova em vez de inflar MechA/MechB, seguindo a estratégia que o
// simulador já usava: MechA cuida dos equilíbrios gasosos, MechB dos de pH
// simples, e a fachada Mech roteia por id de modo (D.MECH_C). Nenhuma linha
// das duas mecânicas existentes foi tocada.
//
// RIGOR DAS CONTAS — nenhuma usa a aproximação preguiçosa:
//  · Kps: a precipitação é resolvida por BISSEÇÃO no avanço, respeitando a
//    estequiometria (CaF₂ é 4s³, Fe(OH)₃ é 27s⁴ — não s²).
//  · Tampão: equação de 2º grau completa do ácido fraco na presença da base
//    conjugada, que se reduz a Henderson-Hasselbalch quando as concentrações
//    são muito maiores que [H⁺] — e continua correta quando não são.
//  · Hidrólise: equação de 2º grau completa do equilíbrio de hidrólise, em vez
//    de √(Kh·C), que erra em concentração baixa.
/** Expoentes em sobrescrito, para escrever Kps = [Ca²⁺]·[F⁻]² em vez de ^2. */
const SUP = { 2: '²', 3: '³', 4: '⁴' };
...
class MechC {
  ...
  build() {
    const D = this.D;
    fillOptGrid('kps-grid', D.SAIS_KPS.map(s => ({
      value: s.id, nome: s.nome, dot: s.cor, extra: s.kps.toExponential(1),
      aria: `${s.nome}, produto de solubilidade ${s.kps.toExponential(1)}. ${s.desc}`,
    })), this.sal.id);
    fillOptGrid('tamp-grid', D.TAMPOES.map(t => ({
      value: t.id, nome: t.nome, dot: t.cor, extra: 'pKa ' + fmt(-Math.log10(t.ka), 2),
      aria: `${t.nome}, pKa ${fmt(-Math.log10(t.ka), 2)}. ${t.uso}`,
    })), this.par.id);
    fillOptGrid('hid-grid', D.SAIS_HIDROLISE.map(s => ({
      value: s.id, nome: s.nome, dot: s.cor, extra: this._hidRotulo(s),
  ...
  setMode(id) {
    this.modo = id;
    if (id === 'tampao') { this.addH = 0; this.histT = []; }
  }
  ...
  setParam(k, v) {
    ...
    switch (k) {
      case 'kpsSal': {
        ...
        this.precAntes = false;
        return { say: `${this.sal.nome}: Kps de ${this.sal.kps.toExponential(2)}. ${this.sal.desc}.` };
      }
      case 'kpsCat': this.kCat = v; return;
      case 'kpsAni': this.kAni = v; return;

      case 'tampPar': {
        this.par = D.TAMPOES.find(t => t.id === v) || this.par;
        this.addH = 0; this.histT = [];
        return { say: `${this.par.nome}, pKa ${fmt(-Math.log10(this.par.ka), 2)}. ${this.par.uso}.` };
      }
      case 'tampHA': this.tHA = v; this.addH = 0; this.histT = []; return;
      case 'tampA':  this.tA = v;  this.addH = 0; this.histT = []; return;
      case 'tampVol': this.tVol = v; this.addH = 0; this.histT = []; return;

      case 'hidSal': {
        this.hSal = D.SAIS_HIDROLISE.find(s => s.id === v) || this.hSal;
        return { say: `${this.hSal.nome}, vindo de ${this.hSal.origem}. ${this.hSal.desc}.` };
      }
      case 'hidC': this.hC = v; return;
    }
    return {};
  }
  ...
  action(name) {
    if (name === 'kps-status') {
      const r = this._kps();
      const veredito = r.Q > r.kps * 1.02
        ? `Q maior que Kps: precipita. Depois de precipitar, sobram ${r.catEq.toExponential(2)} molar de cátion e ${r.aniEq.toExponential(2)} molar de ânion, e o produto volta a valer exatamente o Kps.`
        : r.Q < r.kps * 0.98
          ? 'Q menor que Kps: solução insaturada, nada precipita. Ainda cabe mais sal dissolvido.'
          : 'Q praticamente igual a Kps: solução exatamente saturada, no limite da precipitação.';
      return announce(`${this.sal.nome}. Produto iônico Q igual a ${r.Q.toExponential(2)}, contra Kps de ${r.kps.toExponential(2)}. ${veredito} Solubilidade molar do sal em água pura: ${r.s.toExponential(2)} molar.`, 'assertive');
    }
    if (name === 'kps-saturar') {
    ...
    if (name === 'tamp-acido' || name === 'tamp-base' || name === 'tamp-acido10') {
      ...
      this.histT.push({ n: this.histT.length + 1, pt: r.ph, pa: r.phAgua });
      if (this.histT.length > 60) this.histT.shift();
      playTone(dn > 0 ? 420 : 700, .08, .05);
      const nome = dn > 0 ? `${fmt(Math.abs(dn) * 1000, 0)} milimol de ácido forte` : '1 milimol de base forte';
      return announce(`${nome} adicionado. No tampão o pH foi para ${fmt(r.ph, 2)}, variação de ${fmt(Math.abs(r.ph - r.ph0), 2)}. Na mesma quantidade de água pura o pH iria para ${fmt(r.phAgua, 2)}. ${r.esgotado ? 'ATENÇÃO: o tampão esgotou — um dos componentes acabou e agora o pH desaba como na água.' : ''}`, 'assertive');
    }
    if (name === 'tamp-reset') {
      this.addH = 0; this.histT = [];
      return announce('Tampão reiniciado sem adições.');
    }

    if (name === 'hid-status') {
      const r = this._hidrolise();
      return announce(`${this.hSal.nome}, de ${this.hSal.origem}. Caso: ${r.caso}. pH igual a ${fmt(r.ph, 2)}, ou seja, solução ${r.classe}. ${r.explica}`, 'assertive');
    }
  }
  ...
  _kps() {
    const S = this.sal, [a, b] = S.ec, kps = S.kps;
    const cat0 = Math.pow(10, this.kCat), ani0 = Math.pow(10, this.kAni);
    ...
    return {
      kps, Q, precipita, cat0, ani0, catEq, aniEq, xPrec,
      s: this._sDe(kps, S.ec),
      // razão Q/Kps em escala log: é o que a régua do canvas desenha
      lq: Math.log10(Math.max(1e-45, Q)), lk: Math.log10(kps),
      expr: `[${S.cat}]${a > 1 ? SUP[a] : ''} · [${S.ani}]${b > 1 ? SUP[b] : ''}`,
      sExpr: a === 1 && b === 1 ? 's = √Kps'
        : `s = ${a + b === 3 ? '∛' : (a + b === 4 ? '⁴√' : '')}(Kps/${Math.pow(a, a) * Math.pow(b, b)})`,
    };
  }
  ...
  _tampao() {
    const P = this.par, Kw = this.D.KW, V = this.tVol, ka = P.ka;
    ...
    const nHA0 = this.tHA * V, nA0 = this.tA * V;
    const n = this.addH;
    const nHA = nHA0 + n, nA = nA0 - n;

    const ph0 = this._phDe(this._hMistura(ka, nHA0 / V, nA0 / V), Kw);

    let ph, esgotado = false;
    if (nA <= 1e-12) {
    ...
    return {
      ph, ph0, phAgua, esgotado, pKa,
      nHA: Math.max(0, nHA), nA: Math.max(0, nA), nHA0, nA0, V,
      razao: nHA > 0 ? Math.max(0, nA) / nHA : Infinity,
      dentroFaixa: Math.abs(ph - pKa) <= 1,
      capAcido: Math.max(0, nA0 - Math.max(0, n)) * 1000,   // mmol de H⁺ ainda absorvíveis
      capBase: Math.max(0, nHA0 + Math.min(0, n)) * 1000,   // mmol de OH⁻ ainda absorvíveis
      addMmol: n * 1000,
  ...
  _phDe(h, Kw) {
    ...
    return clamp(-Math.log10(hc), 0, 14);
  }

  /* ══════════════ CONTAS — Hidrólise salina ══════════════ */

  _hidrolise() {
    const S = this.hSal, Kw = this.D.KW, C = Math.pow(10, this.hC);
    let ph = 7, caso, classe, explica, kh = null, ion = '—', x = 0;
    ...
    if (S.catForte && S.aniForte) {
      ...
      ph = clamp(14 + Math.log10(Math.max(1e-14, x)), 0, 14);
      classe = 'básica';
      ion = S.ani;
      ...
      ph = clamp(-Math.log10(Math.max(1e-14, x)), 0, 14);
      classe = 'ácida';
      ion = S.cat;
    ...
    return {
      ph, caso, classe, explica, kh, ion, C, x,
      pOH: 14 - ph,
      h: Math.pow(10, -ph), oh: Kw / Math.pow(10, -ph),
  ...
  _corPh(p) {
    const paleta = ['#dc2626', '#ea580c', '#f59e0b', '#facc15', '#a3e635', '#4ade80',
      '#34d399', '#22d3ee', '#3b82f6', '#6366f1', '#8b5cf6'];
    const t = clamp(p / 14, 0, 1) * (paleta.length - 1);
    const i = Math.floor(t);
    return kMix(paleta[i], paleta[Math.min(paleta.length - 1, i + 1)], t - i);
  }
  ...
  update(dt) {
    ...
    if (this.modo === 'kps') {
      ...
      if (r.precipita && !this.precAntes) { this.precFlash = 1; playTone(330, .12, .06); }
  ...
  draw(ctx, W, H, app) {
    if (this.modo === 'kps') this._dKps(ctx, W, H);
    else if (this.modo === 'tampao') this._dTampao(ctx, W, H);
    else this._dHidrolise(ctx, W, H);
  }

  _dKps(ctx, W, H) {
    const r = this._kps(), S = this.sal, est = isEstreito(W);

    kLabel(ctx, `${S.nome}(s) ⇌ ${S.ec[0] > 1 ? S.ec[0] + ' ' : ''}${S.cat} + ${S.ec[1] > 1 ? S.ec[1] + ' ' : ''}${S.ani}`,
      W / 2, est ? 22 : 30, { size: est ? 12 : 15, bold: true, color: cssVar('--text-primary'), maxW: W - 20 });
    kLabel(ctx, `Kps = ${r.expr} = ${r.kps.toExponential(2)}   (o sólido não entra)`,
      W / 2, est ? 40 : 52, { size: est ? 9 : 11, mono: true, color: cssVar('--text-secondary'), maxW: W - 20 });

    // ── béquer com precipitado ──
    const bw = est ? propW(W, .42, 90, 160) : propW(W, .22, 110, 240);
    const bh = est ? Math.min(H * .34, 170) : Math.min(H * .5, 300);
    const bx = est ? W * .27 : W * .18;
    const by = est ? 66 + bh : H * .30 + bh;
    ctx.save();
    ctx.translate(bx, by);
    kBeaker(ctx, 0, -bh, bw, bh, .8, cssVar('--accent-cyan', '#22d3ee'), { alpha: .3, rotulo: '' });
    // camada de precipitado no fundo, proporcional ao quanto precipitou
    if (r.precipita) {
      const fr = clamp(r.xPrec / Math.max(1e-12, Math.max(r.cat0 / S.ec[0], r.ani0 / S.ec[1])), .06, 1);
      const ph2 = Math.max(4, fr * bh * .3);
      ctx.save();
      ctx.fillStyle = S.cor;
      ctx.globalAlpha = .55 + this.precFlash * .45;
      kRound(ctx, -bw / 2 + 4, -ph2 - 3, bw - 8, ph2, 3); ctx.fill();
      ctx.restore();
      ...
      if (!semMovimento) {
        ctx.save(); ctx.fillStyle = S.cor; ctx.globalAlpha = .7;
        for (let i = 0; i < 9; i++) {
          const t = (this.fase * .5 + i / 9) % 1;
          const x = -bw / 2 + 10 + ((i * 37) % Math.max(1, bw - 20));
          ctx.beginPath(); ctx.arc(x, -bh + t * (bh - ph2 - 6), 2.2, 0, Math.PI * 2); ctx.fill();
        }
        ctx.restore();
      }
    }
    ctx.restore();
    kChip(ctx, r.precipita ? 'PRECIPITA' : (Math.abs(r.Q - r.kps) / r.kps < .02 ? 'saturada' : 'insaturada'),
      bx, by + 20,
      { fg: r.precipita ? cssVar('--accent-exo') : (Math.abs(r.Q - r.kps) / r.kps < .02 ? cssVar('--accent-amber') : cssVar('--accent-ok')),
        size: 11, bold: true });
    ...
    const lo = Math.min(r.lk, r.lq) - 2, hi = Math.max(r.lk, r.lq) + 2;
    const pos = v => gx + clamp((v - lo) / Math.max(.5, hi - lo), 0, 1) * gw;

    ctx.save();
    ctx.strokeStyle = cssVar('--border', '#1c2e44'); ctx.lineWidth = 10; ctx.lineCap = 'round';
    ctx.beginPath(); ctx.moveTo(gx, gy); ctx.lineTo(gx + gw, gy); ctx.stroke();
    ctx.restore();
    // zona de precipitação (à direita do Kps) sombreada
    ctx.save();
    ctx.fillStyle = cssVar('--accent-exo', '#f87171'); ctx.globalAlpha = .13;
    ctx.fillRect(pos(r.lk), gy - 16, gx + gw - pos(r.lk), 32);
    ctx.restore();
    kLabel(ctx, 'insaturada', (gx + pos(r.lk)) / 2, gy - 26, { size: 9, color: cssVar('--accent-ok') });
    kLabel(ctx, 'precipita', (pos(r.lk) + gx + gw) / 2, gy - 26, { size: 9, color: cssVar('--accent-exo') });

    ctx.save();
    ctx.strokeStyle = cssVar('--accent-ok', '#4ade80'); ctx.lineWidth = 3;
    ctx.beginPath(); ctx.moveTo(pos(r.lk), gy - 14); ctx.lineTo(pos(r.lk), gy + 14); ctx.stroke();
    ctx.restore();
    kChip(ctx, `Kps = ${r.kps.toExponential(1)}`, clamp(pos(r.lk), gx + 50, gx + gw - 50), gy + 32,
      { fg: cssVar('--accent-ok'), size: 10, bold: true });

    const cQ = r.precipita ? cssVar('--accent-exo', '#f87171') : cssVar('--accent-amber', '#fbbf24');
    ctx.save(); ctx.fillStyle = cQ;
    ctx.beginPath();
    ctx.moveTo(pos(r.lq), gy - 8); ctx.lineTo(pos(r.lq) - 8, gy - 22); ctx.lineTo(pos(r.lq) + 8, gy - 22);
    ctx.closePath(); ctx.fill(); ctx.restore();
    kChip(ctx, `Q = ${r.Q.toExponential(1)}`, clamp(pos(r.lq), gx + 46, gx + gw - 46), gy - 40,
      { fg: cQ, size: 11, bold: true });

    // ── tabela numérica ──
    let ty = gy + 62;
    const linhas = [
      [`[${S.cat}] inicial`, r.cat0.toExponential(2) + ' mol/L'],
      [`[${S.ani}] inicial`, r.ani0.toExponential(2) + ' mol/L'],
      ['Q (produto iônico)', r.Q.toExponential(3)],
      ['Kps (25 °C)', r.kps.toExponential(3)],
      ['Solubilidade s em água pura', r.s.toExponential(3) + ' mol/L'],
      [r.sExpr, S.ec[0] === 1 && S.ec[1] === 1 ? 'estequiometria 1:1' : `coeficientes ${S.ec[0]}:${S.ec[1]}`],
    ];
    if (r.precipita) {
    ...
    linhas.forEach(l => {
      if (ty > H - 8) return;
      kLabel(ctx, l[0], gx, ty, { size: 10, align: 'left', color: cssVar('--text-secondary'), maxW: tw * .58 });
      kLabel(ctx, l[1], gx + tw, ty, { size: 10, align: 'right', mono: true, bold: true, color: cssVar('--text-primary'), maxW: tw * .4 });
      ty += dy;
    });
  }

  _dTampao(ctx, W, H) {
    const r = this._tampao(), P = this.par, est = isEstreito(W);

    kLabel(ctx, `${P.ha} ⇌ H⁺ + ${P.a}    ·    pKa = ${fmt(r.pKa, 2)}`,
      W / 2, est ? 20 : 28, { size: est ? 12 : 15, bold: true, color: cssVar('--text-primary'), maxW: W - 20 });
    kLabel(ctx, `pH = pKa + log([${P.a}]/[${P.ha}]) = ${fmt(r.pKa, 2)} + log(${fmt(r.nA, 4)}/${fmt(r.nHA, 4)})`,
      W / 2, est ? 36 : 48, { size: est ? 9 : 11, mono: true, color: cssVar('--text-secondary'), maxW: W - 16 });

    // ── dois béqueres: tampão × água pura, MESMA adição ──
    const bw = est ? Math.min(W * .32, 110) : propW(W, .17, 90, 190);
    ...
    const par = [
      { x: x1, ph: r.ph, rot: 'TAMPÃO', sub: `${P.ha} / ${P.a}` },
      { x: x2, ph: r.phAgua, rot: 'ÁGUA PURA', sub: 'sem tampão' },
    ];
    par.forEach(b => {
      ctx.save(); ctx.translate(b.x, yBase);
      kBeaker(ctx, 0, -bh, bw, bh, .78, this._corPh(b.ph), { alpha: .62, rotulo: '' });
      ctx.restore();
      kLabel(ctx, b.rot, b.x, yBase - bh - 16, { size: 10, bold: true, color: cssVar('--text-secondary'), maxW: bw * 1.6 });
      kLabel(ctx, b.sub, b.x, yBase - bh - 4, { size: 9, color: cssVar('--text-muted'), maxW: bw * 1.7 });
      kChip(ctx, `pH ${fmt(b.ph, 2)}`, b.x, yBase + 18,
        { fg: this._corPh(b.ph), size: est ? 11 : 13, bold: true, border: this._corPh(b.ph) });
    });
    // a variação de cada um desde o início — o número que prova o efeito
    kLabel(ctx, `Δ = ${fmt(Math.abs(r.ph - r.ph0), 2)}`, x1, yBase + 38,
      { size: 10, bold: true, mono: true, color: cssVar('--accent-ok') });
    kLabel(ctx, `Δ = ${fmt(Math.abs(r.phAgua - 7), 2)}`, x2, yBase + 38,
      { size: 10, bold: true, mono: true, color: cssVar('--accent-exo') });

    if (r.esgotado) {
      kChip(ctx, '⚠ TAMPÃO ESGOTADO — um dos componentes acabou', W / 2, yBase + 60,
        { fg: cssVar('--accent-exo'), size: 11, bold: true, border: cssVar('--accent-exo') });
    }
    ...
    if (gw > 130 && gh > 80) {
      ...
      const A = kAxes(ctx, {
        x: gx, y: gy, w: gw, h: gh, xmin: 0, xmax: nmax, ymin: 0, ymax: 14,
        xticks: [], yticks: [0, 2, 4, 7, 10, 12, 14],
        xlab: 'adições de ácido/base forte', ylab: 'pH',
      });
      // faixa útil de tamponamento: pKa ± 1
      ctx.save();
      ctx.fillStyle = cssVar('--accent-ok', '#4ade80'); ctx.globalAlpha = .12;
      ctx.fillRect(A.px(0), A.py(Math.min(14, r.pKa + 1)), gw, A.py(Math.max(0, r.pKa - 1)) - A.py(Math.min(14, r.pKa + 1)));
      ctx.restore();
      kLabel(ctx, 'faixa útil: pKa ± 1', A.px(0) + 6, A.py(r.pKa), { size: 9, align: 'left', color: cssVar('--accent-ok') });
      ctx.save();
      ctx.setLineDash([3, 3]); ctx.strokeStyle = cssVar('--accent-ok');
      ctx.beginPath(); ctx.moveTo(A.px(0), A.py(r.pKa)); ctx.lineTo(A.px(nmax), A.py(r.pKa)); ctx.stroke();
      ctx.restore();

      if (this.histT.length > 1) {
        kLine(ctx, this.histT.map(h => [h.n, h.pt]), A.px, A.py, { color: cssVar('--accent-ok', '#4ade80'), w: 2.6 });
        kLine(ctx, this.histT.map(h => [h.n, h.pa]), A.px, A.py, { color: cssVar('--accent-exo', '#f87171'), w: 2.2, dash: [5, 4] });
      }
      kChip(ctx, 'tampão', A.px(nmax * .18), gy + 14, { fg: cssVar('--accent-ok'), size: 9 });
      kChip(ctx, 'água pura', A.px(nmax * .62), gy + 14, { fg: cssVar('--accent-exo'), size: 9 });
      if (this.histT.length < 2) {
        kLabel(ctx, 'Adicione ácido ou base para traçar as duas curvas.',
          gx + gw / 2, gy + gh / 2, { size: 11, color: cssVar('--text-muted'), maxW: gw - 16 });
      }
    }
  }

  _dHidrolise(ctx, W, H) {
    const r = this._hidrolise(), S = this.hSal, est = isEstreito(W);

    kLabel(ctx, `${S.nome} em água`, W / 2, est ? 20 : 28,
      { size: est ? 13 : 16, bold: true, color: cssVar('--text-primary') });
    kLabel(ctx, S.origem, W / 2, est ? 36 : 48,
      { size: est ? 9 : 11, color: cssVar('--text-secondary'), maxW: W - 20 });

    // ── béquer colorido pelo pH ──
    const bw = est ? Math.min(W * .40, 130) : propW(W, .19, 100, 210);
    const bh = est ? Math.min(H * .28, 140) : Math.min(H * .42, 250);
    const bx = est ? W / 2 : W * .22;
    const yBase = est ? 56 + bh : H * .28 + bh;
    ctx.save(); ctx.translate(bx, yBase);
    kBeaker(ctx, 0, -bh, bw, bh, .78, this._corPh(r.ph), { alpha: .62, rotulo: '' });
    // íons hidrolizando: os que reagem piscam, os espectadores ficam apagados
    const total = 20, nAtivos = Math.round(clamp(r.grau * 60, 0, 1) * total);
    for (let i = 0; i < total; i++) {
      const a = i * 2.399 + this.fase * .3;
      const x = Math.cos(a) * (bw * .3), y = -bh * .5 + Math.sin(a * 1.4) * (bh * .25);
      const ativo = i < Math.max(1, nAtivos);
      ctx.fillStyle = ativo ? cssVar('--accent-main', '#f472b6') : cssVar('--text-muted', '#64748b');
      ctx.globalAlpha = ativo ? .95 : .4;
      ctx.beginPath(); ctx.arc(x, y, ativo ? 4 : 3, 0, Math.PI * 2); ctx.fill();
    }
    ctx.globalAlpha = 1;
    ctx.restore();
    kChip(ctx, `pH ${fmt(r.ph, 2)} · ${r.classe}`, bx, yBase + 20,
      { fg: this._corPh(r.ph), size: est ? 11 : 13, bold: true, border: this._corPh(r.ph) });
    ...
    for (let i = 0; i <= 14; i++) g.addColorStop(i / 14, this._corPh(i));
    ctx.save();
    kRound(ctx, sx, sy, sw, 22, 5); ctx.fillStyle = g; ctx.fill();
    ctx.strokeStyle = cssVar('--border'); ctx.lineWidth = 1; ctx.stroke();
    ctx.restore();
    [0, 7, 14].forEach(v => kLabel(ctx, String(v), sx + v / 14 * sw, sy + 32,
      { size: 9, mono: true, color: cssVar('--text-muted') }));
    const mx = sx + r.ph / 14 * sw;
    ctx.save();
    ctx.strokeStyle = '#fff'; ctx.lineWidth = 2.4;
    ctx.beginPath(); ctx.moveTo(mx, sy - 5); ctx.lineTo(mx, sy + 27); ctx.stroke();
    ctx.restore();
    // referência do neutro, para o desvio ser visível
    ctx.save();
    ctx.setLineDash([2, 3]); ctx.strokeStyle = cssVar('--text-muted');
    ctx.beginPath(); ctx.moveTo(sx + sw / 2, sy - 10); ctx.lineTo(sx + sw / 2, sy + 30); ctx.stroke();
    ctx.restore();

    // ── equação da hidrólise e números ──
    let ty = sy + 54;
    const eq = S.catForte && S.aniForte
      ? 'nenhum íon reage com a água'
      : (S.catForte && !S.aniForte)
        ? `${S.ani} + H₂O  ⇌  ${S.ani.replace('⁻', '')}H + OH⁻`
        : (!S.catForte && S.aniForte)
          ? `${S.cat} + H₂O  ⇌  ${S.cat.replace('⁺', '')} + H₃O⁺`
          : `${S.cat} e ${S.ani} hidrolizam ao mesmo tempo`;
    kLabel(ctx, eq, sx, ty, { size: est ? 10 : 12, align: 'left', bold: true, color: cssVar('--accent-main'), maxW: sw });
    ty += 22;
    const linhas = [
      ['Caso', r.caso],
      ['Íon que hidroliza', r.ion],
      ['Concentração do sal', r.C.toExponential(2) + ' mol/L'],
      ['Kh = Kw/K', r.kh == null ? 'não se aplica' : r.kh.toExponential(3)],
      ['[H₃O⁺]', r.h.toExponential(2) + ' mol/L'],
      ['[OH⁻]', r.oh.toExponential(2) + ' mol/L'],
      ['pH · pOH', `${fmt(r.ph, 2)} · ${fmt(r.pOH, 2)}`],
      ['Grau de hidrólise', r.kh == null ? '—' : fmt(r.grau * 100, 3) + ' %'],
    ];
    const dy = Math.max(14, Math.min(22, (H - ty - 12) / linhas.length));
    linhas.forEach(l => {
      if (ty > H - 8) return;
      kLabel(ctx, l[0], sx, ty, { size: 10, align: 'left', color: cssVar('--text-secondary'), maxW: sw * .52 });
      kLabel(ctx, l[1], sx + sw, ty, { size: 10, align: 'right', mono: true, bold: true, color: cssVar('--text-primary'), maxW: sw * .46 });
      ty += dy;
    });
  }
  ...
  getResults() {
    if (this.modo === 'kps') {
      const r = this._kps(), S = this.sal;
      const rows = [
        { l: 'Sal', v: S.nome },
        { l: 'Dissociação', v: `${S.nome}(s) ⇌ ${S.ec[0] > 1 ? S.ec[0] + ' ' : ''}${S.cat} + ${S.ec[1] > 1 ? S.ec[1] + ' ' : ''}${S.ani}` },
        { l: 'Expressão de Kps', v: r.expr },
        { l: 'Kps (25 °C)', v: r.kps.toExponential(3), cls: 'val-ok' },
        { l: `[${S.cat}]`, v: r.cat0.toExponential(3) + ' mol/L' },
        { l: `[${S.ani}]`, v: r.ani0.toExponential(3) + ' mol/L' },
        { l: 'Q (produto iônico)', v: r.Q.toExponential(3), cls: r.precipita ? 'val-exo' : 'val-endo' },
        { l: 'Veredito', v: r.precipita ? 'Q > Kps: PRECIPITA' : (Math.abs(r.Q - r.kps) / r.kps < .02 ? 'Q = Kps: saturada' : 'Q < Kps: insaturada'), cls: r.precipita ? 'val-exo' : 'val-ok' },
        { l: 'Solubilidade molar s', v: r.s.toExponential(3) + ' mol/L' },
        { l: 'Como s sai do Kps', v: r.sExpr },
      ];
      if (r.precipita) {
        rows.push({ l: 'Precipitou', v: r.xPrec.toExponential(3) + ' mol/L de sal', cls: 'val-exo' });
        rows.push({ l: `[${S.cat}] no equilíbrio`, v: r.catEq.toExponential(3) + ' mol/L' });
        rows.push({ l: `[${S.ani}] no equilíbrio`, v: r.aniEq.toExponential(3) + ' mol/L' });
        rows.push({ l: 'Q após precipitar', v: 'volta a valer o Kps', cls: 'val-ok' });
      }
      rows.push({ l: 'Aplicação', v: S.desc });
      return rows;
    }
    if (this.modo === 'tampao') {
      const r = this._tampao(), P = this.par;
      return [
        { l: 'Par tampão', v: `${P.ha} / ${P.a}` },
        { l: 'Ka', v: P.ka.toExponential(2) },
        { l: 'pKa', v: fmt(r.pKa, 2), cls: 'val-ok' },
        { l: 'Volume', v: fmt(r.V, 1) + ' L' },
        { l: `n(${P.ha}) atual`, v: fmt(r.nHA * 1000, 2) + ' mmol' },
        { l: `n(${P.a}) atual`, v: fmt(r.nA * 1000, 2) + ' mmol' },
        { l: 'Razão [A⁻]/[HA]', v: isFinite(r.razao) ? fmt(r.razao, 3) : '∞' },
        { l: 'pH do tampão', v: fmt(r.ph, 3), cls: 'val-ok' },
        { l: 'pH inicial (sem adição)', v: fmt(r.ph0, 3) },
        { l: 'Variação no tampão', v: fmt(Math.abs(r.ph - r.ph0), 3) + ' unidade(s)', cls: 'val-ok' },
        { l: 'pH da água pura', v: fmt(r.phAgua, 3), cls: 'val-exo' },
        { l: 'Variação na água', v: fmt(Math.abs(r.phAgua - 7), 3) + ' unidade(s)', cls: 'val-exo' },
        { l: 'Total adicionado', v: `${fmt(r.addMmol, 2)} mmol de ${r.addMmol >= 0 ? 'H⁺' : 'OH⁻'}` },
        { l: 'Ainda absorve de H⁺', v: fmt(r.capAcido, 2) + ' mmol' },
        { l: 'Ainda absorve de OH⁻', v: fmt(r.capBase, 2) + ' mmol' },
        { l: 'Dentro da faixa útil?', v: r.dentroFaixa ? 'sim (pKa ± 1)' : 'não — fora de pKa ± 1', cls: r.dentroFaixa ? 'val-ok' : 'val-exo' },
        { l: 'Estado', v: r.esgotado ? 'ESGOTADO' : 'tamponando', cls: r.esgotado ? 'val-exo' : 'val-ok' },
        { l: 'Onde isso aparece', v: P.uso },
      ];
    }
    const r = this._hidrolise(), S = this.hSal;
    return [
      { l: 'Sal', v: S.nome },
      { l: 'Origem', v: S.origem },
      { l: 'Caso de hidrólise', v: r.caso, cls: 'val-ok' },
      { l: 'Íon que hidroliza', v: r.ion },
      { l: 'Concentração', v: r.C.toExponential(3) + ' mol/L' },
      { l: 'Kw (25 °C)', v: '1,0·10⁻¹⁴' },
      { l: 'Kh = Kw/K', v: r.kh == null ? 'não se aplica (nenhum íon hidroliza)' : r.kh.toExponential(3) },
      { l: '[H₃O⁺]', v: r.h.toExponential(3) + ' mol/L', cls: 'val-exo' },
      { l: '[OH⁻]', v: r.oh.toExponential(3) + ' mol/L', cls: 'val-endo' },
      { l: 'pH', v: fmt(r.ph, 3), cls: 'val-ok' },
      { l: 'pOH', v: fmt(r.pOH, 3) },
      { l: 'Caráter', v: r.classe, cls: 'val-ok' },
      { l: 'Grau de hidrólise', v: r.kh == null ? '—' : fmt(r.grau * 100, 4) + ' %' },
      { l: 'Por quê', v: r.explica },
    ];
  }
  ...
  getOverlay() {
    ...
    if (this.modo === 'tampao') {
      const r = this._tampao();
      return `tampão pH ${fmt(r.ph, 2)} · água pH ${fmt(r.phAgua, 2)}`;
    }
    const r = this._hidrolise();
    return `${this.hSal.nome} · pH ${fmt(r.ph, 2)} (${r.classe})`;
  }

  onArrow(dx) {
    if (!dx) return false;
    if (this.modo === 'kps') {
      this.kAni = clamp(this.kAni + dx * .1, -8, 0);
      this.app.syncSlider('kps-ani', this.kAni);
      return true;
    }
    if (this.modo === 'hidrolise') {
      this.hC = clamp(this.hC + dx * .05, -3, 0);
...
class Mech {
  constructor(D) {
    ...
    this.a = new MechA(D);
    this.b = new MechB(D);
    this.c = new MechC(D);          // Kps, tampao e hidrolise
    this._bSet = new Set(D.MECH_B || []);
    this._cSet = new Set(D.MECH_C || []);
    this.cur = this.a;
  }
  set app(v) { this._app = v; this.a.app = v; this.b.app = v; this.c.app = v; }
