...
SISOL.kRound = function kRound(ctx, x, y, w, h, r) {
...
SISOL.kLabel = function kLabel(ctx, text, x, y, o = {}) {
  ctx.save();
...
SISOL.kChip = function kChip(ctx, text, x, y, o = {}) {
  ctx.save();
...
SISOL.kArrow = function kArrow(ctx, x1, y1, x2, y2, o = {}) {
  const head = o.head || 7,
    ang = Math.atan2(y2 - y1, x2 - x1);
  ctx.save();
  ctx.strokeStyle = ctx.fillStyle = o.color || SISOL.cssVar('--text-secondary');
 ...
 */
SISOL.kAxes = function kAxes(ctx, cfg) {
  const {
    x,
    y,
    w,
  ...
  const grid = SISOL.cssVar('--border', '#1c2e44'),
    txt = SISOL.cssVar('--text-muted', '#3d566e');
  ctx.save();
  ctx.lineWidth = 1;
  (cfg.xticks || []).forEach(t => {
    ...
    ctx.stroke();
    ctx.globalAlpha = 1;
    SISOL.kLabel(ctx, cfg.fmtx ? cfg.fmtx(t) : SISOL.fmt(t, 0), px(t), y + h + 11, {
      size: 10,
      color: txt,
      mono: true
    });
  ...
  (cfg.yticks || []).forEach(t => {
    ...
    ctx.stroke();
    ctx.globalAlpha = 1;
    SISOL.kLabel(ctx, cfg.fmty ? cfg.fmty(t) : SISOL.fmt(t, 0), x - 6, py(t), {
      size: 10,
      color: txt,
      align: 'right',
      mono: true
  ...
  ctx.lineTo(x + w, y + h);
  ctx.stroke();
  if (cfg.xlab) SISOL.kLabel(ctx, cfg.xlab, x + w / 2, y + h + 26, {
    size: 11,
...
SISOL.kLine = function kLine(ctx, pts, px, py, o = {}) {
  if (!pts.length) return;
  ctx.save();
  ctx.strokeStyle = o.color || SISOL.cssVar('--accent-main');
 ...
 */
SISOL.kBeaker = function kBeaker(ctx, cx, topY, w, h, level, liquidColor, o = {}) {
  const x = cx - w / 2,
    glass = SISOL.cssVar('--glass', 'rgba(148,163,184,.38)');
  ...
  if (lh > 1) {
    ...
    SISOL.kRound(ctx, x + 3, ly, w - 6, lh, 4);
    ctx.fill();
    // menisco
    ctx.globalAlpha = .5;
  ...
  ctx.stroke();
  if (o.rotulo) SISOL.kLabel(ctx, o.rotulo, cx, topY + h + 14, {
    size: 11,
    color: SISOL.cssVar('--text-secondary')
  });
  ctx.restore();
...
SISOL.kThermo = function kThermo(ctx, x, topY, h, t, tmin, tmax, o = {}) {
  const frac = Math.max(0, Math.min(1, (t - tmin) / (tmax - tmin)));
  const bulbR = 8,
    tubeW = 7;
  const tubeTop = topY,
    tubeBot = topY + h - bulbR * 2;
  const merc = o.color || SISOL.cssVar('--accent-exo', '#f87171');
  ctx.save();
  ctx.fillStyle = SISOL.cssVar('--bg-void', '#080c14');
  ctx.strokeStyle = SISOL.cssVar('--glass');
  ctx.lineWidth = 2;
  SISOL.kRound(ctx, x - tubeW / 2, tubeTop, tubeW, h - bulbR, tubeW / 2);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
  ...
  ctx.fillStyle = merc;
  SISOL.kRound(ctx, x - 2.2, tubeBot - mh, 4.4, mh + bulbR, 2.2);
  ctx.fill();
  for (let i = 0; i <= 4; i++) {
    ...
    ctx.stroke();
    if (o.escala !== false) SISOL.kLabel(ctx, SISOL.fmt(tmin + i / 4 * (tmax - tmin), 0), x + tubeW / 2 + 9, yy, {
      size: 9,
  ...
  if (o.rotulo !== false) SISOL.kChip(ctx, `${SISOL.fmt(t, o.casas != null ? o.casas : 0)} °C`, x, tubeTop - 14, {
    bg: 'rgba(0,0,0,.45)',
    fg: merc,
    size: 11,
    bold: true
...
SISOL.kFlame = function kFlame(ctx, x, y, s, time) {
  const flu = SISOL.isReduced() ? 0 : Math.sin(time * 9) * s * .06;
  ctx.save();
  const g = ctx.createRadialGradient(x, y - s * .5, s * .1, x, y - s * .5, s);
  g.addColorStop(0, SISOL.cssVar('--flame-b', '#fde047'));
  g.addColorStop(1, SISOL.cssVar('--flame-a', '#f97316'));
  ctx.fillStyle = g;
  ctx.beginPath();
  ctx.moveTo(x - s * .45, y);
...
SISOL.kParticles = function kParticles(arr, n, box, speed, dt) {
  ...
  const v = SISOL.isReduced() ? 0 : speed;
  arr.forEach(p => {
    p.x += p.vx * v * dt;
    p.y += p.vy * v * dt;
    if (p.x < box.x) {
...
SISOL.kDrawParticles = function kDrawParticles(ctx, arr, r, color, alpha) {
  ctx.save();
  ctx.fillStyle = color;
...
SISOL.kBubbles = function kBubbles(arr, dt, box, rate, o = {}) {
  if (!SISOL.isReduced() && Math.random() < rate * dt) {
    arr.push({
...
SISOL.kDrawBubbles = function kDrawBubbles(ctx, arr, color) {
  ctx.save();
...
SISOL.kFlowDots = function kFlowDots(ctx, pts, phase, n, color, o = {}) {
  const segs = [];
  let total = 0;
  ...
  for (let k = 0; k < n; k++) {
    ...
    for (const s of segs) {
      if (d <= s.L) {
        ...
        ctx.fill();
        if (o.rotulo && k === 0) SISOL.kLabel(ctx, 'e⁻', s.p[0] + s.dx * t, s.p[1] + s.dy * t - 9, {
          size: 9,
          color,
          mono: true
        });
...
SISOL.kMix = function kMix(h1, h2, t) {
  const p = h => [parseInt(h.slice(1, 3), 16), parseInt(h.slice(3, 5), 16), parseInt(h.slice(5, 7), 16)];
  const a = p(h1),
    b = p(h2);
  return `rgb(${Math.round(SISOL.lerp(a[0], b[0], t))},${Math.round(SISOL.lerp(a[1], b[1], t))},${Math.round(SISOL.lerp(a[2], b[2], t))})`;
};
/** Interpolação linear em tabela de pontos [[x,y],...] ordenada por x. */
SISOL.kInterp = function kInterp(pts, x) {
...
// ══════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════
// KIT DE DESENHO — extensões exclusivas de Soluções
// (além dos helpers comuns em nucleo/kit-desenho.js: gotas caindo,
//  floco de gelo, vapor e torneira.)
// ══════════════════════════════════════════════════════════════════
SISOL.kDrops = function kDrops(arr, dt, o) {
  if (!SISOL.isReduced() && Math.random() < (o.rate || 6) * dt) {
    arr.push({
      x: o.x + (Math.random() - .5) * (o.spread || 4),
      y: o.y,
...
SISOL.kDrawDrops = function kDrawDrops(ctx, arr, color) {
  ctx.save();
  ctx.fillStyle = color || 'rgba(147,197,253,.85)';
  arr.forEach(d => {
    ctx.beginPath();
    ctx.ellipse(d.x, d.y, d.r * .6, d.r, 0, 0, Math.PI * 2);
    ctx.fill();
  });
  ctx.restore();
};
/** Floco de gelo simples (asterisco de 3 traços) — sinaliza congelamento. */
SISOL.kSnowflake = function kSnowflake(ctx, x, y, s, color) {
  ctx.save();
...
SISOL.kSteam = function kSteam(ctx, x, y, phase, color) {
  ctx.save();
  ctx.strokeStyle = color;
  ctx.globalAlpha = .5;
  ctx.lineWidth = 1.6;
...
SISOL.kTap = function kTap(ctx, x, y, aberta, cor) {
  ctx.save();
  ctx.strokeStyle = SISOL.cssVar('--glass');
  ctx.lineWidth = 3;
  ctx.lineCap = 'round';
  ...
  ctx.strokeStyle = cor || SISOL.cssVar('--accent-amber');
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(0, 0);
  ctx.lineTo(0, -11);
