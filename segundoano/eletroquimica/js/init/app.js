...
SIELQ.fillOptGrid = function fillOptGrid(gridId, items, selValue) {
...
SIELQ.App = class App {
  constructor(mech) {
    ...
    this.refresh();
    SIELQ.announce(`${this.D.ACRO} carregado. Nenhum modo ativo. Escolha um modo à esquerda e ative-o para começar.`);
  ...
  _resize() {
    ...
    SIELQ.patchCtxFont(this.ctx);
    SIELQ.CANVAS_FS = SIELQ.canvasFS(this.W);
    this.lay = SIELQ.layoutMode(this.W);
  }
  ...
  _syncMobileModeUI(id) {
    ...
    if (activeTab && typeof activeTab.scrollIntoView === 'function') {
      activeTab.scrollIntoView({ inline: 'center', block: 'nearest', behavior: SIELQ.isReduced() ? 'auto' : 'smooth' });
    }
  ...
  clearMode() {
    ...
    if (typeof SIELQ.playTone === 'function') SIELQ.playTone(420, .06, .05);
    SIELQ.announce('Modo desativado. Nenhum modo ativo — escolha um modo e ative-o para voltar a simular.');
  }
  setMode(id, silent) {
    const m = this.D.MODES.find(x => x.id === id);
    if (!m) return;
    ...
    if (!silent) {
      SIELQ.playTone(760, .06, .05);
      SIELQ.announce(`Modo ${m.nome} selecionado. A animação começa em 2 segundos. ${(m.info || '').split('.')[0]}.`);
    }
  }
  ...
  _bindPanelArea(sb) {
    sb.addEventListener('click', e => {
      ...
      if (hdr) {
        ...
        SIELQ.playTone(exp ? 500 : 750, .06, .04);
        return;
      }
  ...
  fmtOut(inp, v) {
    ...
    // um slider logarítmico varre várias ordens de grandeza, e um número
    // fixo de casas arredondava valores pequenos para "0" — escondendo
    // a grandeza real da concentração.
    const txt = ehLog ? SIELQ.fmtCientifico(val, 1) : SIELQ.fmt(val, casas);
    return txt + (inp.dataset.unit ? ' ' + inp.dataset.unit : '');
  }
  _param(k, v) {
    const r = this.mech.setParam(k, v) || {};
    if (r.warn) SIELQ.announce(r.warn, 'assertive');
    if (r.say) SIELQ.announce(r.say);
    this.refresh();
  }
  ...
  _bindHeader() {
    ...
    if (logo) logo.addEventListener('click', () => {
      ...
      SIELQ.playTone(660, .09, .06);
      SIELQ.announce('Você sabia? ' + fato);
    });
    document.addEventListener('keydown', e => {
      if (!e.altKey) return;
  ...
  _loop() {
    ...
    const dt = SIELQ.clamp((now - this._last) / 1000, 0, .05);
    this._last = now;
...
window.addEventListener('DOMContentLoaded', () => new SIELQ.App(new SIELQ.Mech(window.SIM_DATA)));