...
SIELQ.canvasFS = function canvasFS(W) {
  let a11y = 1;
  try {
...
SIELQ.kFont = function kFont(spec) {
  if (typeof spec !== 'string' || SIELQ.CANVAS_FS === 1) return spec;
  return spec.replace(/(\d+(?:\.\d+)?)px/, (m, n) => Math.round(parseFloat(n) * SIELQ.CANVAS_FS * 10) / 10 + 'px');
};
...
SIELQ.patchCtxFont = function patchCtxFont(ctx) {
  ...
  try {
    ...
    Object.defineProperty(ctx, 'font', {
      ...
      set(v) {
        desc.set.call(ctx, SIELQ.kFont(v));
      },
      configurable: true
    });
...
SIELQ.layoutMode = function layoutMode(W) {
  return W < 620 ? 'estreito' : W < 1000 ? 'normal' : 'largo';
};
SIELQ.isEstreito = W => W < 620;
...
SIELQ.propW = function propW(W, frac, min_, max_) {
  const v = W * frac;
  return max_ == null ? Math.max(min_, v) : Math.min(max_, Math.max(min_, v));
};
SIELQ.lerp = (a, b, t) => a + (b - a) * t;
SIELQ.easeIO = t => t < .5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2;
SIELQ.isReduced = () => document.body.classList.contains('reduce-motion');
/** Lê uma variável de cor do CSS (fonte única de cores do simulador). */
SIELQ.cssVar = function cssVar(name, fallback = '#fb923c') {
  const v = getComputedStyle(document.body).getPropertyValue(name).trim();
  return v || fallback;
};
/** Contraste preto/branco por luminância YIQ — réplica do SIMA/SILQ. */
SIELQ.getContrastColor = function getContrastColor(hex) {
  const c = hex.replace('#', '');
