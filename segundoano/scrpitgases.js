...
class MixEngine {
  ...
  sigmaOf(M) { return KIN.VSCALE * this.vBoost * Math.sqrt(Math.max(1, this.T) / Math.max(0.1, M)); }
  ...
  _ajustaBoost() {
    if (!this.specs.length) { this.vBoost = 1; return; }
    const Mmax = Math.max(...this.specs.map(s => s.M));
    const sigmaBase = KIN.VSCALE * Math.sqrt(Math.max(1, this.T) / Math.max(0.1, Mmax));
    const alvo = 34;                      // px/s minimos para a especie mais lenta
    this.vBoost = clamp(alvo / Math.max(1e-6, sigmaBase), 1, 7);
  }
  ...
  fracOf(i) { return this._pFracSp[i] || 0; }
  ...
  config(o) {
    if (o.T != null && o.T !== this.T) { this.T = o.T; this._ajustaBoost(); }
    if (o.r != null) this.r = o.r;
  }

  _spawnOne(spIdx) {
    const M = this.specs[spIdx].M;
    const s = this.sigmaOf(M), b = this.box, r = this.r;
  ...
  setMix(specs) {
    ...
    this.parts.forEach(p => { if (atual[p.sp] != null) atual[p.sp]++; });
    // remove excedentes (varre de trás pra frente)
    for (let i = this.parts.length - 1; i >= 0; i--) {
      const sp = this.parts[i].sp;
      if (sp >= specs.length || atual[sp] > alvo[sp]) { atual[sp]--; this.parts.splice(i, 1); }
    }
    // cria faltantes
    specs.forEach((sp, i) => {
      while (atual[i] < alvo[i]) { this.parts.push(this._spawnOne(i)); atual[i]++; }
    });
    if (this._pFracSp.length !== specs.length) this._pFracSp = specs.map(() => 0);
    if (this._reamostrarDepois) { this._reamostrarDepois = false; this.resample(); }
  }
  ...
  ecOf(spIdx) {
    let s = 0, n = 0;
    this.parts.forEach(p => { if (p.sp === spIdx) { s += p.M * (p.vx * p.vx + p.vy * p.vy); n++; } });
    return n ? s / n : 0;
  }
  ...
  _substep(dt) {
    ...
    for (const p of this.parts) {
      ...
      let imp = 0;
      if (p.x < b.x + r) { p.x = b.x + r; if (p.vx < 0) { imp += 2 * p.M * Math.abs(p.vx); p.vx = -p.vx; } }
      else if (p.x > b.x + b.w - r) { p.x = b.x + b.w - r; if (p.vx > 0) { imp += 2 * p.M * Math.abs(p.vx); p.vx = -p.vx; } }
      if (p.y < b.y + r) { p.y = b.y + r; if (p.vy < 0) { imp += 2 * p.M * Math.abs(p.vy); p.vy = -p.vy; } }
      else if (p.y > b.y + b.h - r) { p.y = b.y + b.h - r; if (p.vy > 0) { imp += 2 * p.M * Math.abs(p.vy); p.vy = -p.vy; } }
    ...
    this.specs.forEach((sp, i) => {
      ...
      const tau = clamp(2.2 * L / Math.max(1e-6, this.sigmaOf(sp.M)), 0.5, 5);
      const k = 1 - Math.exp(-dt / tau);
      this._pFracSp[i] = lerp(this._pFracSp[i] || 0, inst, k);
    });
    let collCount = 0;
    if (this.collisions && this.parts.length > 1) collCount = this._collideGrid();
    this._collRate = lerp(this._collRate, collCount / Math.max(dt, 1e-6), kSmTot);
    ...
    this.specs.forEach((sp, i) => {
      ...
      if (sg <= 0) return;
      let s2 = 0, n = 0;
      this.parts.forEach(p => { if (p.sp === i) { s2 += p.vx * p.vx + p.vy * p.vy; n++; } });
      if (!n) return;
      ...
      const sc = clamp(Math.sqrt(1 + (dt / 0.25) * (1 / ratio - 1)), 0.88, 1.12);
      this.parts.forEach(p => { if (p.sp === i) { p.vx *= sc; p.vy *= sc; } });
    });
  }
  ...
  _collideGrid() {
    ...
    const buckets = new Map();
    this.parts.forEach((p, i) => {
      let cx = clamp(Math.floor((p.x - b.x) / cell), 0, cols - 1);
      let cy = clamp(Math.floor((p.y - b.y) / cell), 0, rows - 1);
      const k = cy * cols + cx;
      if (!buckets.has(k)) buckets.set(k, []);
      buckets.get(k).push(i);
    });
...
class Mech {
  constructor(D) {
    ...
    this.mix = {
      a: gN2, b: gO2, c: null,
      na: 2.34, nb: 0.63, nc: 0,      // ≈ 78 % e 21 % em fracao molar
      T: 300, V: 24.6, r: 2.5,   // ver nota sobre excesso de gas real em _mixCalc
      view: 'dalton',
      eng: new MixEngine(),
    };
    this._grReset();
  ...
  build() {
    ...
    fillSelect('mix-a', selItems, this.mix.a.id);
    fillSelect('mix-b', selItems, this.mix.b.id);
    fillSelect('mix-c', [{ value: '', nome: '— nenhum —' }].concat(selItems), '');
    this._buildPares();
    this._buildMisturas();
  }
  ...
  setParam(k, v) {
    const T = this.tr, C = this.cl, G = this.gr, K = this.kin, X = this.mix, D = this.D;
    switch (k) {
      case 'trtipo': {
        T.tipo = v;
      ...
      /* ── modo Mistura ── */
      case 'mixA': {
        X.a = D.GASES.find(g => g.id === v) || X.a; X.eng.resample();
        return { say: `Gás A da mistura: ${X.a.nome}, massa molar ${fmt(X.a.M, 2)}.` };
      }
      case 'mixB': {
        X.b = D.GASES.find(g => g.id === v) || X.b; X.eng.resample();
        return { say: `Gás B da mistura: ${X.b.nome}, massa molar ${fmt(X.b.M, 2)}.` };
      }
      case 'mixC': {
        ...
        if (X.c && X.nc <= 0) { X.nc = 0.5; this.app.syncSlider('mix-nc', 0.5); }
        if (!X.c) X.nc = 0;
        X.eng.resample();
        return { say: X.c ? `Gás C: ${X.c.nome}. Mistura ternária.` : 'Gás C removido. Mistura binária.' };
      }
      case 'mixNa': X.na = v; return;
      case 'mixNb': X.nb = v; return;
      case 'mixNc': X.nc = v; return;
      case 'mixT': X.T = v; X.eng.config({ T: v }); return;
      case 'mixV': X.V = v; return;
      case 'mixview':
        X.view = v;
        return { say: v === 'dalton'
          ? 'Exibindo a Lei de Dalton: coluna de pressões parciais empilhadas.'
          : 'Exibindo a Lei de Amagat: barra de volumes parciais e a comparação de energia cinética.' };
    }
  }
  ...
  action(name, el) {
    ...
    if (name === 'tr-reset') {
      ...
      if (!c.itens.length) { announce('Nenhum gás na mistura. Dê mols a pelo menos um deles.', 'assertive'); return; }
      const detalhe = c.itens.map(it =>
        `${it.g.nome}: fração molar ${fmt(it.x, 3)}, parcial de ${fmt(it.PiMed, 2)} atmosferas`).join('; ');
      playTone(700, .08, .06);
      announce(`Pressão total ${fmt(c.Ptot, 2)} atmosferas. ${detalhe}. A soma das parciais medidas dá ${fmt(c.somaMed, 2)} atmosferas — Lei de Dalton confirmada. Massa molar aparente da mistura: ${fmt(c.Mbar, 2)} gramas por mol.`, 'assertive');
    } else if (name === 'mix-reset') {
      const X = this.mix, D = this.D;
      X.a = D.GASES.find(g => g.id === 'n2') || X.a;
      X.b = D.GASES.find(g => g.id === 'o2') || X.b;
      X.c = null;
      Object.assign(X, { na: 2.34, nb: 0.63, nc: 0, T: 300, V: 24.6, view: 'dalton' });
      this._mixSyncUI();
      X.eng.resample();
      playTone(440, .07, .05);
      announce('Mistura reiniciada no ar atmosférico: 78 por cento de nitrogênio e 21 por cento de oxigênio.');
      ...
      X.a = acha(p.itens[0][0]) || X.a; X.na = p.itens[0][1];
      X.b = acha(p.itens[1][0]) || X.b; X.nb = p.itens[1][1];
      if (p.itens[2]) { X.c = acha(p.itens[2][0]); X.nc = p.itens[2][1]; }
      else { X.c = null; X.nc = 0; }
      this._mixSyncUI();
      X.eng.resample();
      playTone(600, .07, .05);
      announce(`${p.rot} carregada. ${p.tag}.`);
    }
  }
  ...
  onArrow(dx, dy) {
    ...
    if (this.mode === 'mistura') {
      ...
      const X = this.mix;
      if (dy) { X.T = clamp(X.T - dy * 10, 100, 900); this.app.syncSlider('mix-t', X.T); X.eng.config({ T: X.T }); }
      if (dx) { X.V = clamp(X.V + dx * 0.5, 10, 60); this.app.syncSlider('mix-v', X.V); }
      return !!(dx || dy);
    }
    return false;
  }
  ...
  update(dt, app) {
    ...
    if (!isReduced()) {
      ...
      else if (this.mode === 'mistura') this.mix.eng.step(dt);
    }
  }
  ...
  draw(ctx, W, H, app) {
    ...
    else if (this.mode === 'mistura') this._dMistura(ctx, W, H, app);
  ...
  _dTransform(ctx, W, H, app) {
    ...
    const cw = propW(W, est ? .34 : .26, 120, W * .42);
    const ch = H * (est ? .44 : .58), top = H * (est ? .09 : .16);
    const cx = est ? W * .42 : W * .34;
    this._cylinder(ctx, cx, top, cw, ch, c.V, 50, c.T, app, cssVar('--accent-main'));
    kThermo(ctx, est ? W * .08 : W * .11, top + 8, ch * .8, c.T - 273, -273, 327,
      { escala: false, casas: 0, rotulo: false });
    const gr = propW(W, .09, 38, 96);
    if (est) this._gauge(ctx, W * .5, Math.min(H - gr - 14, top + ch + gr + 24), gr, c.P, 4, cssVar('--accent-amber'));
    else     this._gauge(ctx, W * .72, H * .34, gr, c.P, 4, cssVar('--accent-amber'));
  }
  ...
  _dClape(ctx, W, H, app) {
    ...
    const est = isEstreito(W);
    const cw = propW(W, est ? .38 : .3, 130, W * .46);
    const ch = H * (est ? .42 : .56), top = H * (est ? .09 : .16);
    ...
    ctx.restore();
    const gr2 = propW(W, .09, 38, 96);
    if (est) this._gauge(ctx, W * .5, Math.min(H - gr2 - 14, top + ch + gr2 + 24), gr2, c.P, 6, cssVar('--accent-amber'));
    else     this._gauge(ctx, W * .74, H * .3, gr2, c.P, 6, cssVar('--accent-amber'));
  }
  ...
  _mixCalc() {
    const X = this.mix, D = this.D, R = D.R;
    ...
    const itens = ativos.map((it, i) => {
      ...
      return {
        g: it.g, n: it.n, x,
        Pi: x * Ptot,                    // Dalton:  Pᵢ = xᵢ·P
        Vi: x * X.V,                     // Amagat:  Vᵢ = xᵢ·V
        PiMed: X.eng.fracOf(i) * (it.n * R * X.T / X.V),   // parcial MEDIDA
        vrms: vrms3D(X.T, it.g.M, D.RSI),
        ec: X.eng.ecOf(i),
        idx: i,
      };
    });
    ...
    return {
      itens, nTot, Ptot, Mbar,
      d: X.T > 0 ? Ptot * Mbar / (R * X.T) : 0,          // densidade da mistura
  ...
  _mixSync() {
    ...
    X.eng.config({ T: X.T, r: X.r });
    X.eng.setMix(ativos.map((it, i) => ({
      id: it.g.id, M: it.g.M, cor: it.g.cor, n: contagens[i],
    })));
  }

  _dMistura(ctx, W, H, app) {
    const X = this.mix, c = this._mixCalc();
    const est = isEstreito(W);

    // ── recipiente ──
    const cw = propW(W, est ? .82 : .46, 200, W * (est ? .88 : .52));
    ...
    const box = { x: cx - cw / 2 + 6, y: top + 6, w: cw - 12, h: ch - 12 };
    this._mixSync();
    X.eng.setBox(box);
    ...
    X.eng.parts.forEach(p => {
      ...
      if (!sp) return;
      const r = clamp(2.2 + Math.sqrt(sp.M) * .17, 2.4, 5.4);
      ctx.fillStyle = sp.cor; ctx.globalAlpha = .9;
      ctx.beginPath(); ctx.arc(p.x, p.y, r, 0, Math.PI * 2); ctx.fill();
    });
    ctx.restore();

    if (!c.itens.length) {
      kLabel(ctx, 'Escolha os gases e dê mols a pelo menos um deles.', cx, top + ch / 2,
        { size: 13, color: cssVar('--text-secondary'), maxW: cw - 20 });
      return;
    }
    ...
    if (X.view === 'dalton') this._mixBarrasDalton(ctx, c, bx, by, bw, bh, est);
    else this._mixBarrasAmagat(ctx, c, X, bx, by, bw, bh, est);

    // rodapé: a verificação numérica da lei
    const ok = c.desvio < .06;
    kChip(ctx,
      `ΣPᵢ medida = ${fmt(c.somaMed, 2)} atm  =  P total medida ${fmt(c.PtotMed, 2)} atm  ✓ Dalton`,
      W / 2, H - (est ? 30 : 32),
      { fg: ok ? cssVar('--accent-ok') : cssVar('--accent-amber'), size: 11, bold: true,
        border: ok ? cssVar('--accent-ok') : cssVar('--accent-amber') });
    ...
    kLabel(ctx,
      `Clapeyron para a mistura: ${fmt(c.Ptot, 2)} atm  ·  medido ${c.excessoReal >= 0 ? '+' : ''}${fmt(c.excessoReal * 100, 0)} % (volume próprio das partículas — o “b” de van der Waals)`,
      W / 2, H - 10, { size: 9, color: cssVar('--text-muted'), maxW: W - 16 });
  }
  ...
  _mixBarrasDalton(ctx, c, bx, by, bw, bh, est) {
    ...
    ctx.save();
    ctx.fillStyle = cssVar('--bg-hover');
    kRound(ctx, colX, by, colW, bh, 5); ctx.fill();
    ctx.strokeStyle = cssVar('--border'); ctx.lineWidth = 1.2;
    kRound(ctx, colX, by, colW, bh, 5); ctx.stroke();
    ctx.restore();

    // escala do eixo
    for (let i = 0; i <= 4; i++) {
      const v = pmax * i / 4, y = by + bh - (v / pmax) * bh;
      kLabel(ctx, fmt(v, 1), colX - 8, y, { size: 9, align: 'right', mono: true, color: cssVar('--text-muted') });
      ctx.save(); ctx.strokeStyle = cssVar('--border'); ctx.globalAlpha = .5;
      ctx.beginPath(); ctx.moveTo(colX, y); ctx.lineTo(colX + colW, y); ctx.stroke(); ctx.restore();
    }
    ...
    c.itens.forEach(it => {
      ...
      const y = by + bh - acc - h;
      ctx.save();
      ctx.fillStyle = it.g.cor; ctx.globalAlpha = .82;
      kRound(ctx, colX + 2, y, colW - 4, Math.max(1, h), 3); ctx.fill();
      ctx.restore();
      if (h > 15) {
        kLabel(ctx, `${it.g.f}  ${fmt(it.PiMed, 2)} atm`, colX + colW / 2, y + h / 2,
          { size: 10, bold: true, color: getContrastColor(it.g.cor), maxW: colW - 8 });
      }
      acc += h;
    });
    kLabel(ctx, 'PRESSÕES PARCIAIS (medidas)', colX + colW / 2, by - 16,
      { size: 10, bold: true, color: cssVar('--text-secondary'), maxW: colW * 1.6 });
    ...
    c.itens.forEach(it => {
      ...
      ctx.beginPath(); ctx.arc(tx + 5, ty, 4.2, 0, Math.PI * 2); ctx.fill();
      ctx.restore();
      kLabel(ctx, `${it.g.f}  x = ${fmt(it.x, 3)}`, tx + 16, ty,
        { size: 10, align: 'left', mono: true, color: cssVar('--text-primary'), maxW: tw - 20 });
      ty += dy * .62;
      kLabel(ctx, `Pᵢ = xᵢ·P = ${fmt(it.Pi, 2)} atm`, tx + 16, ty,
        { size: 9, align: 'left', mono: true, color: cssVar('--text-muted'), maxW: tw - 20 });
      ty += dy;
    });
    kLabel(ctx, `Σxᵢ = ${fmt(c.itens.reduce((a, i) => a + i.x, 0), 3)}  (sempre 1)`, tx, ty,
      { size: 10, align: 'left', mono: true, color: cssVar('--accent-ok'), maxW: tw });
    ty += dy * .7;
    kLabel(ctx, `M̄ = ${fmt(c.Mbar, 2)} g/mol  ·  d = ${fmt(c.d, 3)} g/L`, tx, ty,
      { size: 10, align: 'left', mono: true, color: cssVar('--text-secondary'), maxW: tw });
  }
  ...
  _mixBarrasAmagat(ctx, c, X, bx, by, bw, bh, est) {
    ...
    ctx.save();
    ctx.fillStyle = cssVar('--bg-hover');
    kRound(ctx, bx, y, bw, barH, 6); ctx.fill();
    ctx.restore();
    let acc = 0;
    c.itens.forEach(it => {
      const w = it.x * bw;
      ctx.save();
      ctx.fillStyle = it.g.cor; ctx.globalAlpha = .82;
      kRound(ctx, bx + acc, y, Math.max(1, w), barH, 4); ctx.fill();
      ctx.restore();
      if (w > 34) {
        kLabel(ctx, it.g.f, bx + acc + w / 2, y + barH / 2 - 6,
          { size: 11, bold: true, color: getContrastColor(it.g.cor) });
        kLabel(ctx, `${fmt(it.Vi, 1)} L`, bx + acc + w / 2, y + barH / 2 + 7,
          { size: 9, mono: true, color: getContrastColor(it.g.cor) });
      }
      acc += w;
    });
    ctx.save();
    ctx.strokeStyle = cssVar('--border'); ctx.lineWidth = 1.2;
    kRound(ctx, bx, y, bw, barH, 6); ctx.stroke();
    ctx.restore();
    kLabel(ctx, `VOLUMES PARCIAIS (Amagat) — total ${fmt(X.V, 1)} L`, bx + bw / 2, y - 14,
      { size: 10, bold: true, color: cssVar('--text-secondary'), maxW: bw });
    kLabel(ctx, 'cada gás ocupa o recipiente TODO: Vᵢ é o volume que ele ocuparia sozinho à pressão total',
      bx + bw / 2, y + barH + 18,
      { size: 9, color: cssVar('--text-muted'), maxW: bw });

    // velocidades e energia cinética: o argumento de por que a parcial não
    // depende da massa molar
    let ty = y + barH + 44;
    kLabel(ctx, 'À mesma T: velocidades diferentes, MESMA energia cinética média',
      bx + bw / 2, ty, { size: 10, bold: true, color: cssVar('--accent-cyan'), maxW: bw });
    ty += 20;
    const ecMax = Math.max(1e-9, ...c.itens.map(i => i.ec));
    c.itens.forEach(it => {
      kLabel(ctx, `${it.g.f}  M = ${fmt(it.g.M, 1)} g/mol  ·  v_rms = ${fmt(it.vrms, 0)} m/s`,
        bx, ty, { size: 10, align: 'left', mono: true, color: it.g.cor, maxW: bw * .72 });
      // barrinha de energia cinética média medida — todas praticamente iguais
      const w = (it.ec / ecMax) * (bw * .24);
      ctx.save();
      ctx.fillStyle = cssVar('--accent-cyan'); ctx.globalAlpha = .55;
      kRound(ctx, bx + bw * .74, ty - 5, Math.max(2, w), 10, 3); ctx.fill();
      ctx.restore();
      ty += 19;
    });
    kLabel(ctx, '↑ as barrinhas de Ec ficam do mesmo tamanho — é isso que Dalton exige',
      bx, ty + 2, { size: 9, align: 'left', color: cssVar('--text-muted'), maxW: bw });
  }
  ...
  getResults() {
    ...
    if (this.mode === 'mistura') {
      const X = this.mix, c = this._mixCalc();
      if (!c.itens.length) return [{ l: 'Mistura', v: 'vazia — dê mols a um gás' }];
      const rows = [
        { l: 'Temperatura', v: `${fmt(X.T, 0)} K` },
        { l: 'Volume total', v: `${fmt(X.V, 1)} L` },
        { l: 'n total', v: `${fmt(c.nTot, 2)} mol` },
        { l: 'P total (nRT/V)', v: `${fmt(c.Ptot, 2)} atm`, cls: 'val-ok' },
      ];
      c.itens.forEach(it => {
        rows.push({ l: `x (${it.g.f})`, v: fmt(it.x, 4) });
        rows.push({ l: `P${it.g.f} = x·P`, v: `${fmt(it.Pi, 3)} atm` });
        rows.push({ l: `P${it.g.f} medida`, v: `${fmt(it.PiMed, 3)} atm`, cls: 'val-endo' });
        rows.push({ l: `V${it.g.f} = x·V (Amagat)`, v: `${fmt(it.Vi, 2)} L` });
        rows.push({ l: `v_rms (${it.g.f})`, v: `${fmt(it.vrms, 0)} m/s` });
      });
      rows.push({ l: 'Σ pressões parciais', v: `${fmt(c.somaMed, 2)} atm`, cls: 'val-ok' });
      rows.push({ l: 'P total medida', v: `${fmt(c.PtotMed, 2)} atm`, cls: 'val-ok' });
      rows.push({ l: 'Dalton confere?', v: c.desvio < .06 ? `sim (desvio ${fmt(c.desvio * 100, 1)} %)` : `medindo… (${fmt(c.desvio * 100, 1)} %)`, cls: c.desvio < .06 ? 'val-ok' : '' });
      rows.push({ l: 'Medido − ideal', v: `${c.excessoReal >= 0 ? '+' : ''}${fmt(c.excessoReal * 100, 1)} %` });
      rows.push({ l: 'Por que sobra?', v: 'volume próprio das partículas (gás real)' });
      rows.push({ l: 'M̄ aparente', v: `${fmt(c.Mbar, 2)} g/mol`, cls: 'val-endo' });
      rows.push({ l: 'Massa da mistura', v: `${fmt(c.massa, 1)} g` });
      rows.push({ l: 'Densidade d = P·M̄/RT', v: `${fmt(c.d, 3)} g/L` });
      rows.push({ l: 'Σ frações molares', v: fmt(c.itens.reduce((a, i) => a + i.x, 0), 3) });
      return rows;
    }
  ...
  getOverlay() {
    ...
    if (this.mode === 'mistura') { const c = this._mixCalc(); return `mistura · ${fmt(c.Ptot, 2)} atm · M̄ ${fmt(c.Mbar, 1)}`; }
    const Pm = this._kinPressure();
...
class App {
  ...
  _loop() {
    ...
    if (this.mode) {
      ...
      if (this.mode.id === 'cinetica' || (this.mode.id === 'graham' && this.mech.gr.run)) {
