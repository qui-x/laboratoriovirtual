/* ═══════════════════════════════════════════════════════════════
   ...
   ───────────────────────────────────────────────────────────────
   Cria a alça de arraste nas bordas internas das sidebars esquerda/
   direita, permitindo redimensioná-las (largura salva em
   localStorage e restaurada na próxima visita). Registra seu próprio
   listener de DOMContentLoaded, independente de main.js.
   Depende de: nada além do HTML/CSS (variáveis --sl/--sr).
   Usado por: carregado diretamente pela página; não é chamado por
              outros módulos.
═══════════════════════════════════════════════════════════════ */
...
document.addEventListener('DOMContentLoaded', function () {
  var targets = [{ id:'sidebar-left', side:'left', cssVar:'--sl', min:240, max:560 },{ id:'sidebar-right', side:'right', cssVar:'--sr', min:240, max:560 }];
  ...
  targets.forEach(function (cfg) {
    ...
    var storeKey = 'sime-w-' + cfg.cssVar.replace(/^--/, '');
    try {
