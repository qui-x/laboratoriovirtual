/* ═══════════════════════════════════════════════════════════════
   CAMADA: INTERFACE (independente do resto do SIQI)
   ARQUIVO: sidebar-resizer.js
   ───────────────────────────────────────────────────────────────
   Alça de arraste nas bordas das sidebars, com largura salva em
   localStorage — mesmo padrão do SILQ/SIEM/SIME/SIE/SIFI.
   ...
   900px — ver comentário logo acima de `.sidebar--left { width:
   var(--sl); ... }` em stylesiqi.css), mas nenhum arquivo JS criava
   a alça nem escutava o arraste: o SIQI era, junto com o SIFI, o
   único da família sem redimensionar de sidebar. Este arquivo fecha
   essa lacuna, sem alterar nada do CSS que já existia.
   Depende de: nada além do HTML/CSS.
   Usado por: carregado diretamente pela página (ver indexsiqi.html).
═══════════════════════════════════════════════════════════════ */
...
document.addEventListener('DOMContentLoaded', function () {
  var targets = [
    { id: 'sidebar-left',  side: 'left',  cssVar: '--sl', min: 220, max: 460 },
    { id: 'sidebar-right', side: 'right', cssVar: '--sr', min: 200, max: 420 }
  ];
