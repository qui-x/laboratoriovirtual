/* ════════════════════════════════════════════════════════════════════
   SILQ — view3dsilq.js  v3   (integrado ao SIQI)
   ════════════════════════════════════════════════════════════════════
   Renderizador 3D autossuficiente do SIMULADOR DE LIGAÇÕES QUÍMICAS
   (SILQ), reaproveitado aqui como motor principal de desenho da
   "Estrutura Molecular" na Ficha de cada composto do SIQI — a mesma
   ideia de integração entre simuladores da plataforma, em vez de
   duplicar um motor de desenho à parte.

   Cópia quase-verbatim do arquivo original: a ÚNICA mudança é em
   isLight()/isHiContrast() (ver comentário na função), pra ler a
   convenção de tema do SIQI (atributos data-theme/data-contrast no
   <html>) em vez da convenção do SILQ (classes no <body>). Todo o
   resto — geometria 3D, câmera orbital, projeção, cunhas
   estereoquímicas, HUD — é idêntico ao original.

   A "ponte" de dados (window.SILQ_VIEW3D_STATE) que alimenta este
   arquivo com os átomos/ligações de cada composto do SIQI vive em
   js/render/silq-integracao.js — só ELE é código novo; este arquivo
   continua sendo o motor de desenho do SILQ, inalterado.
   ────────────────────────────────────────────────────────────────────
...
// .light-mode: classe que o a11y.js do SILQ liga. Antes era 'theme-light',
// nome do painel local antigo do SILQ — o visualizador 3D nunca detectava
// o modo claro e desenhava sempre com as cores do tema escuro.
//
// ── ÚNICA ADAPTAÇÃO deste arquivo em relação ao original do SILQ ──
// O SIQI marca tema/contraste como ATRIBUTOS no <html> (data-theme=
// "light", data-contrast="high" — ver js/a11y/preferencias.js), não
// como CLASSES no <body> como o SILQ. Só isLight()/isHiContrast()
// foram adaptadas para ler a convenção do SIQI; o resto do arquivo
// (toda a geometria 3D, câmera, projeção, desenho) é idêntico ao
// original — motor 100% reaproveitado, zero duplicação de lógica.
function isLight()    { return document.documentElement.getAttribute('data-theme') === 'light'; }
function isHiContrast(){ return document.documentElement.getAttribute('data-contrast') === 'high'; }
...
/* ─────────────────────────────────────────────────────────────────────
   5. INICIALIZAÇÃO
───────────────────────────────────────────────────────────────────── */
/* SEGUNDA adaptação necessária deste arquivo em relação ao original do
   SILQ (a primeira foi isLight()/isHiContrast(), lá em cima) — expõe a
   classe globalmente. No SILQ original, #viewer3d nunca é destruído
   depois de criado, então só precisa de UMA instância, criada aqui.
   No SIQI, a Ficha do composto é reconstruída via innerHTML toda vez
   que o usuário alterna entre "desafio bloqueado" (Nomenclatura) e
   "ficha completa" — isso apaga o <canvas> que o motor criou aqui e
   PERDE a referência (bug real relatado pelo usuário: a caixa
   "Estrutura Molecular" ficava em branco em vários compostos depois
   de navegar pelo app). window.SilqView3D exposto aqui permite que
   js/render/silq-integracao.js recrie o canvas e a instância sempre
   que detectar que o anterior foi destruído — ver
   silqGarantirCanvas3D() lá. */
window.SilqView3D = SilqView3D;
