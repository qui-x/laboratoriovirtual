...
/* ════════════════════════════════════════════════════════════════
   ...
   de duplicar:
     ...
     · .panel[data-open] + initPaineis() — accordion de abrir/fechar
       (o painel do módulo 2 usa a MESMA marcação .panel >
       .panel-header + .panel-body, então herda o abrir/fechar de
       graça, sem código extra aqui)
     · CATALOGO_SIQI, DESAFIOS_CONSTRUTOR — dados vindos de js/data/*
...
/* Painéis que somem quando um módulo específico está ativo (ex.:
   ...
   "construtor,nomenclatura" se um dia precisar esconder de mais de um. */
...
function trocarModulo(nome){
  ...
  /* Área central "gera" o conteúdo do módulo ativo (mesmo padrão do
     SIMA: canvas mostra o modelo selecionado). Construtor tem sua
     própria view central; ao entrar em Nomenclatura vindo de "nenhum
     módulo" ou do Construtor, volta pra Ficha (se já há composto
     carregado) ou Lab — nunca fica preso na dica "escolha um módulo"
     ou no conteúdo do módulo anterior. */
  if(window._setView){
    if(nome === 'construtor'){
      window._setView('construtor');
    } else if(STATE.modoView === 'construtor' || STATE.modoView === 'none'){
